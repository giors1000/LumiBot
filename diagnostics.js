...
function initCollapsibleSections() {
    try {
        const saved = JSON.parse(localStorage.getItem('diag-collapsed') || '[]');
...
function initActionButtons() {
    ...
    document.getElementById('forceOtaBtn')?.addEventListener('click', () => {
        if (DiagState.connected) {
            MQTTClient.publishControl(DiagState.deviceId, { command: 'checkOTA' });
            Toast.success('OTA check command sent');
        } else {
            Toast.error('Device is offline');
        }
    });
...
function analyzeDiagnostics() {
    ...
    if (data.system) {
        ...
        if (MathFree < 60) {
            issues.push(`⚠️ <strong>Critical Memory</strong>: Free heap is critically low (${MathFree}KB). System may become unstable. A reboot is highly recommended.`);
        }
    }
    ...
    if (data.wifi) {
        if (!data.wifi.connected) {
            issues.push(`🌐 <strong>Network</strong>: Device is currently disconnected from WiFi.`);
        }
        if (data.wifi.rssi < -80) {
            issues.push(`📶 <strong>Signal Strength</strong>: Weak WiFi signal (${data.wifi.rssi}dBm). Consider moving the router closer or reducing 2.4GHz interference.`);
        }
        if (data.wifi.disconnectCount > 10) {
            issues.push(`🔄 <strong>Network Stability</strong>: High number of WiFi disconnects (${data.wifi.disconnectCount}). The device might be on the edge of your router's coverage.`);
        }
    }

    // Check MQTT
    if (data.mqtt && data.mqtt.publishFail > 20) {
        issues.push(`☁️ <strong>Cloud Connection</strong>: High number of MQTT publish failures (${data.mqtt.publishFail}). Ensure the broker is reachable and port 1883/8883 is open in your firewall.`);
    }

    // Device specific checks
    if (data.deviceType === 'stepper') {
        if (data.stepper && data.stepper.suspectedStalls > 0) {
            issues.push(`⚙️ <strong>Motor Stalls</strong>: Detected ${data.stepper.suspectedStalls} potential stall(s). The blinds may be physically jammed or the acceleration profile is too steep. Consider increasing relaxSteps or lowering acceleration.`);
        }
        if (data.stepper && !data.stepper.isCalibrated) {
            issues.push(`📐 <strong>Calibration</strong>: The blinds are not calibrated. Soft limits are disabled.`);
        }
        if (data.touch && data.touch.falseTriggerRate > 2) {
            issues.push(`🖐️ <strong>Touch Interference</strong>: High touch false trigger rate (${data.touch.falseTriggerRate.toFixed(1)}%). EMF from the stepper motor might be triggering the metal touch plate. Try twisting the motor wires or decreasing touch sensitivity.`);
        }
    } else {
        // Zaylo Radar
        if (data.light && data.light.falseTriggerRate > 5) {
            issues.push(`🏃 <strong>Presence Sensor</strong>: High false trigger rate (${data.light.falseTriggerRate.toFixed(1)}%). The radar might be responding to moving curtains, fans, or pets. Consider opening standard settings to reduce Radar Sensitivity by 15-20%.`);
        }
        // Zaylo Relay
        if (data.light && data.light.servoCycles && data.light.servoCycles > 100000) {
             issues.push(`⚡ <strong>Hardware Degradation Warning</strong>: High relay cycle count (${data.light.servoCycles}). Relay contacts may begin degrading soon.`);
        }
    }

    let resultHtml = '';
    if (issues.length === 0) {
        resultHtml = `<div style="color: var(--success); margin-bottom: 8px; font-weight: 500; font-size: 15px;">&#10003; System appears entirely healthy!</div><div style="color: var(--text-secondary); line-height: 1.5; margin-bottom: 15px;">All telemetry metrics are within expected thresholds. High reliability is expected.</div>`;
    } else {
        resultHtml = `<div style="margin-bottom: 12px; color: var(--text-secondary);">Found <strong>${issues.length}</strong> potential issue(s) that might require your attention:</div>`;
        resultHtml += issues.map(issue => `
            <div style="background: rgba(255,255,255,0.06); padding: 14px; border-radius: 10px; margin-bottom: 10px; border-left: 3px solid var(--warning); line-height: 1.5; color: var(--text-primary);">
                ${issue}
            </div>
        `).join('');
    }

    resultHtml += `<div style="margin-top: 20px; font-size: 13px; color: var(--text-tertiary); text-align: center;">Diagnostics are based on the latest telemetry snapshot.</div>`;

    Modal.alert('Auto-Diagnostics Report', resultHtml);
}

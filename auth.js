/**
 * Zaylo - Firebase Authentication Module
...
const Auth = {
    ...
    async _doInit() {
        ...
        try {
            ...
            this._sendPasswordResetEmail = (auth, email) => auth.sendPasswordResetEmail(email);
    ...
    async sendPasswordReset(email) {
        ...
        if (window.location.protocol === 'file:') {
            console.log('[Auth] Mocking password reset email for local testing:', email);
            return new Promise(resolve => setTimeout(() => resolve({ success: true }), 1000));
        }

        if (!this._sendPasswordResetEmail) {
            console.error('[Auth] Firebase not initialized properly - sendPasswordResetEmail not available');
            return { success: false, error: 'Authentication service unavailable. Please refresh and try again.' };
        }

        try {
            await this._sendPasswordResetEmail(this.auth, email);
    ...
    async signOut() {
        ...
        try {
            // CRITICAL: Clear home caching so next user doesn't see ghost data
            try {
                localStorage.removeItem('zaylo-activeHomeId');
                if (typeof HomeService !== 'undefined') HomeService.clearCache();
            } catch(e) {}

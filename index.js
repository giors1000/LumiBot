...
function renderSkeletonLoader(list) {
  list.innerHTML = '';
  // Generate 2 skeleton cards with staggered animation
  for (let i = 0; i < 2; i++) {
  ...
  // Always add the "Add Device" card even during skeleton loading
  const addCard = createAddDeviceCard();
  addCard.style.animationDelay = `${2 * 0.1}s`;
...
async function loadFirebaseDevicesInBackground(list, emptyState, countEl) {
  try {
    ...
    const user = Auth.getUser();
    if (!user) {
      // Not authenticated - just show the add card (no skeletons)
    ...
    // ALWAYS re-render after Firebase sync to clear skeleton loader
    // This is critical - even if needsUpdate is false, we may have skeleton showing
    renderDeviceList(firebaseDevices, list, emptyState, countEl);

  } catch (error) {
    console.error('[Index] Background Firebase sync error:', error);
    // On error, still render what we have from local storage to clear skeleton
    const localDevices = DeviceList.getAll();
    renderDeviceList(localDevices, list, emptyState, countEl);
  }
}

...
function updateBlindDeviceCard(deviceId, state) {
  ...
  if (hasPendingTarget) displayPosition = pendingTarget;
  ...
  const statusText = isConnecting ? 'Connecting...' : (isOnline ? 'Online' : 'Offline');
  const posText = hasPendingTarget ? `Pending ${_badgeLabel(displayPosition)}` : _posText(displayPosition);
  ...
  const badge = card.querySelector('.blind-position-badge');
  if (badge) {
    badge.textContent = hasPendingTarget ? `Pending ${_badgeLabel(displayPosition)}` : _badgeLabel(displayPosition);
    badge.classList.remove('open', 'closed', 'half');
  ...
  if (posFill) {
    ...
    posFill.classList.toggle('pending', hasPendingTarget);
  }
      ...
      content: `
        ...
        <div style="margin-bottom: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <p style="font-size:12px; color:var(--text-tertiary); margin-top:6px;">Default: app host (${Utils.escapeHtml(defaultBrokerHost)})</p>
            </div>
            <div style="margin-bottom: 16px;">

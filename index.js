...
async function initMQTT() {
  ...
  MQTTClient.on('onConnect', async () => {
    ...
    console.log('[Index] MQTT Connected. Subscribing to devices...');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('settingsBtn')?.addEventListener('click', () => {
    const currentIP = localStorage.getItem('LumiBot-BrokerIP') || '192.168.0.137';
    const currentPort = localStorage.getItem('LumiBot-BrokerPort') || '9001';

    const { modal, close } = Modal.create({
      title: 'Connection Settings',
      content: `
        <div style="margin-bottom: 16px;">
          <label style="display:block; color:var(--text-secondary); margin-bottom:8px; font-size:14px;">MQTT Broker IP</label>
          <input type="text" id="brokerIpInput" value="${currentIP}" style="width:100%; padding:12px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:monospace;">
          <p style="font-size:12px; color:var(--text-tertiary); margin-top:6px;">Default: 192.168.0.137</p>
        </div>
        <div style="margin-bottom: 16px;">
          <label style="display:block; color:var(--text-secondary); margin-bottom:8px; font-size:14px;">Websocket Port</label>
          ...
          <p style="font-size:12px; color:var(--text-tertiary); margin-top:6px;">Default: 9001 (WebSockets)</p>
        </div>
        <div style="padding:12px; background:rgba(239,68,68,0.1); border-radius:12px; border:1px solid rgba(239,68,68,0.2);">
           <p style="color:var(--danger); font-size:12px; line-height:1.4;">⚠️ Ensure your device is on the same network as the broker.</p>
        </div>
      `,
      actions: [
        { label: 'Cancel', primary: false },
        {
          ...
          onClick: () => {
            ...
            if (ip) {
              ...
              localStorage.setItem('LumiBot-BrokerPort', port || '9001');

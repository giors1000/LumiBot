...
async function showHomeSettingsModal() {
  ...
  modal.querySelectorAll('.home-member-remove').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      Modal.confirm('Remove Member', `Remove ${Utils.escapeHtml(memberName)} from this home?`, async () => {
  ...
  modal.querySelector('#scanHomeQrBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      stopScannerUi(false);
      setHomeQrScannerStatus(modal, error.message || 'QR scanning is unavailable. Enter the code manually.', 'error');
    }
  });
  ...
  modal.querySelector('#leaveHomeBtn')?.addEventListener('click', () => {
    Haptic.heavy();
    Modal.confirm('Leave Home', `Are you sure you want to leave "${safeHomeName}"? You will lose access to all shared devices.`, async () => {

...
async function initMQTT() {
  ...
  MQTTClient.on('onConnect', async () => {
    ...
    console.log('[Index] MQTT Connected. Stabilizing...');

    // CRITICAL FIX: Clean up invalid devices from LocalStorage before subscribing
    // Junk data like "test", "undefined", or long strings cause Broker Disconnects (Code 8)
    // CRITICAL FIX: Clean up invalid devices from LocalStorage before subscribing
    // Junk data like "test", "undefined", or long strings cause Broker Disconnects (Code 8)
    const devices = DeviceList.getAll();
    const validDevices = devices.filter(d => {
      // Clean the ID first to ensure we aren't rejecting valid IDs with whitespace
      if (!d.id) return false;

      // Aggressively clean: remove anything that is NOT A-F or 0-9
      const cleanId = d.id.toString().replace(/[^A-Fa-f0-9]/g, '').toUpperCase();

      const isValid = /^[A-F0-9]{4}$/.test(cleanId);

      if (!isValid) {
        console.warn(`[Index] Rejected invalid device ID format: "${d.id}" (Cleaned: "${cleanId}")`);
        // Debug: Log character codes to find invisible hidden chars
        const charCodes = Array.from(d.id.toString()).map(c => c.charCodeAt(0));
        console.warn(`[Index] Invalid ID Char Codes: [${charCodes.join(', ')}]`);
      } else {
        // Update the ID in the object to ensure subscription uses the clean version
        if (d.id !== cleanId) {
    ...
    if (devices.length !== validDevices.length) {
      console.warn('[Index] Found invalid devices. Cleaning up...');
      // Optional: DeviceList.set(validDevices); // Uncomment to auto-delete bad data
    }

    if (validDevices.length === 0) {
      console.warn('[Index] No valid devices to subscribe to.');
    }

    // CRITICAL FIX: Stagger subscriptions. 
    // Flooding the socket instantly after connect causes "Socket Closed"
    validDevices.forEach((device, index) => {
      setTimeout(() => {
        if (MQTTClient.connected) {
          console.log(`[Index] Subscribing to sane device ID: ${device.id}`);

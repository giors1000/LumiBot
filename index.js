...
function renderDeviceList(devices, list, emptyState, countEl) {
  ...
  devices.forEach((device, index) => {
    ...
    // Apply staggered animation delay to EVERY card rendered
    // Max out the stagger at 5 items so it doesn't take forever to load a huge list
    const staggerDelay = Math.min(index, 5) * 0.08;
    card.style.animationDelay = `${staggerDelay}s`;
    // Ensure the animation actually runs if it was stripped by the drag logic
  ...
  try {
    ...
    deviceSubscription = await DeviceService.subscribeToDevices(user.uid, (rawDevices) => {
      ...
      if (localJSON !== mergedJSON || localDevices.length !== mergedDevices.length || hasSkeletons) {
        ...
        Storage.set('LumiBot-devices', mergedDevices);

...
function stopBlindGroup() {
  ...
  if (!isConnected || !canPublish) {
    if (typeof Toast !== 'undefined') Toast.error("Can't stop blinds while offline");
    return;
  }
  ...
  blindDevices.forEach(device => {
    const deviceId = String(device.id || '').trim();
    if (!deviceId) return;
    MQTTClient.publishStepperControl(deviceId, { command: 'stop' });
    const pos = _getBlindPositionForDevice(device);
    if (pos !== null) {
      _blindTargetLock.set(deviceId, { target: pos, timestamp: Date.now() });
...
async function initMQTT() {
  ...
  MQTTClient.on('onConnect', async () => {
    ...
    if (typeof BlindSchema !== 'undefined' && typeof BlindSchema.flushPendingConfigSyncs === 'function') {
    ...
    if (!_stateUpdateRafId) {
      _stateUpdateRafId = requestAnimationFrame(() => {
        _pendingStateUpdates.forEach((pendingState, pendingId) => {
          if (typeof BlindSchema !== 'undefined' && typeof BlindSchema.handleConfigAck === 'function') {

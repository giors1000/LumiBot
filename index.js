...
async function setupFirebaseSubscription(list, emptyState, countEl) {
  try {
    ...
    const user = Auth.getUser();
    if (!user) {
      console.debug('[Index] Not authenticated, keeping local devices');
    ...
    deviceSubscription = await DeviceService.subscribeToDevices(user.uid, (rawDevices) => {
      ...
      if (localJSON !== mergedJSON || localDevices.length !== mergedDevices.length || hasSkeletons) {
        ...
        Storage.set('zaylo-devices', mergedDevices);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (devicesChanged) {
    ...
    Storage.set('zaylo-devices', validDevices);

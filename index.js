...
const OPENWEATHER_API_KEY = 'e20c6a9a37404d9726371c943d7da0ba';

// ============================================
// Global Location Auto-Detection
// ============================================
const GlobalLocation = {
  AUTO_ATTEMPT_KEY: 'zaylo-LocationAutoAttemptAt',
  DENIED_KEY: 'zaylo-LocationDeniedAt',
  ATTEMPT_COOLDOWN_MS: 12 * 60 * 60 * 1000,
  DENIED_COOLDOWN_MS: 7 * 24 * 60 * 60 * 1000,

  hasStoredLocation() {
    return Boolean(localStorage.getItem('zaylo-LocationLat') && localStorage.getItem('zaylo-LocationLon'));
  },

  canAutoDetect() {
    if (this.hasStoredLocation()) return false;
    if (!navigator.geolocation) return false;
    if (!window.isSecureContext && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') return false;

    const now = Date.now();
    const lastAttempt = parseInt(localStorage.getItem(this.AUTO_ATTEMPT_KEY) || '0', 10);
    if (lastAttempt && now - lastAttempt < this.ATTEMPT_COOLDOWN_MS) return false;

    const lastDenied = parseInt(localStorage.getItem(this.DENIED_KEY) || '0', 10);
    if (lastDenied && now - lastDenied < this.DENIED_COOLDOWN_MS) return false;

    return true;
  },

  tryAutoDetect() {
    if (!this.canAutoDetect()) return;
    localStorage.setItem(this.AUTO_ATTEMPT_KEY, String(Date.now()));

    navigator.geolocation.getCurrentPosition(
      (pos) => this.applyFromCoords(pos.coords.latitude, pos.coords.longitude, { silent: true }),
      (err) => {
        if (err && err.code === 1) {
          localStorage.setItem(this.DENIED_KEY, String(Date.now()));
        }
        if (window.DEBUG) console.warn('[GlobalLocation] Auto-detect skipped:', err);
      },
      { enableHighAccuracy: true, timeout: 12000, maximumAge: 10 * 60 * 1000 }
    );
  },

  async applyFromCoords(lat, lon, options = {}) {
    let cityName = options.cityName || 'Device Location';
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.name) cityName = data.name;
      }
    } catch (e) {
      if (window.DEBUG) console.warn('[GlobalLocation] Reverse geocode failed:', e);
    }

    this.apply(lat, lon, cityName, options);
  },

  apply(lat, lon, cityName, options = {}) {
  ...
  broadcast(lat, lon) {
    ...
    const devices = (typeof DeviceList !== 'undefined' && DeviceList.getAll) ? DeviceList.getAll() : [];
    devices.forEach(d => {
      if (!d.id) return;
...
const WidgetLayout = {
  ...
  showLargeReorderHint() {
    const banner = document.getElementById('reorderBanner');
    if (!banner) return;
    if (this._largeHintTimer) clearTimeout(this._largeHintTimer);
    banner.textContent = 'Hold to reorder widgets';
    banner.classList.add('visible');
    this._largeHintTimer = setTimeout(() => {
      banner.classList.remove('visible');
      this._largeHintTimer = null;
    }, 2600);
  },
...
const DeviceRescue = {
  ...
  async connect() {
    ...
    try {
      ...
      if (e && e.name === 'NotFoundError') {
        ...
        this.els.subtitle.textContent = "No device found — waiting for rescue mode";
  ...
  async startWifiScan() {
    ...
    try {
      ...
      // Fresh scan → fresh reassembly buffer (results now arrive in chunks).
      this.wifiResultsBuffer = '';
      if (this._wifiResultsTimer) { clearTimeout(this._wifiResultsTimer); this._wifiResultsTimer = null; }
  ...
  handleWifiResults(e) {
    ...
    // The firmware streams the FULL network list in ~128-byte notification
    // chunks (the old single notification truncated dense scans to ~6 networks,
    // silently hiding the user's own SSID). Accumulate until the buffer parses
    // as complete JSON; a quiet-period timer salvages a usable partial list if
    // a trailing chunk is lost mid-air.
    this.wifiResultsBuffer = (this.wifiResultsBuffer || '') + chunk;
    if (this._wifiResultsTimer) { clearTimeout(this._wifiResultsTimer); this._wifiResultsTimer = null; }
    ...
    const tryParse = (buf) => {
      ...
      try { const v = JSON.parse(buf); return Array.isArray(v) ? v : null; } catch (err) { return null; }
    };

    const complete = tryParse(this.wifiResultsBuffer);
    if (complete) {
      this.wifiResultsBuffer = '';
      resetScanBtn();
      this.renderWifiList(complete);
      return;
    }

    // Incomplete — wait briefly for the next chunk before attempting salvage.
    this._wifiResultsTimer = setTimeout(() => {
      this._wifiResultsTimer = null;
      const buf = this.wifiResultsBuffer || '';
      this.wifiResultsBuffer = '';
      let salvaged = tryParse(buf);
      if (!salvaged) {
        const lastClose = buf.lastIndexOf('}');
        if (lastClose > 1) salvaged = tryParse(buf.substring(0, lastClose + 1) + ']');
      }
      resetScanBtn();
      if (salvaged) {
        console.warn('[Rescue] Salvaged partial Wi-Fi scan results');
        this.renderWifiList(salvaged);
      } else {
        console.error('Invalid WiFi results: could not assemble chunks');
...
const DeviceContextMenu = {
  ...
  show(deviceId) {
    ...
    const state = (typeof MQTTClient !== 'undefined' && MQTTClient.getDeviceState)
      ? MQTTClient.getDeviceState(deviceId)
      : null;

    if (!device) return;

    const deviceType = String(device.type || 'lumibot').toLowerCase();
    ...
    const icon = this.menu.querySelector('.context-menu-icon');
    if (icon) {
      icon.classList.toggle('blind', isBlind);
      icon.innerHTML = isBlind
  ...
  handleAction(action) {
    ...
    switch (action) {
      ...
      case 'settings':
        {
          const device = DeviceList.get(deviceId);
          const isBlindOrStepper = _isBlindDevice(device);
          window.location.href = isBlindOrStepper
            ? `blind-device.html?id=${deviceId}`
            : `device.html?id=${deviceId}`;
        }
...
function createDeviceCard(device, state = null, index = 0) {
  ...
  const statusText = isConnecting
    ? (state?._waitingForLiveState ? 'Waiting for state' : 'Connecting...')
...
function _getBlindPendingTarget(deviceId) {
  const saved = _readSavedBlindState(deviceId);
  const pending = Number(saved.pendingTargetPosition);
  return Number.isFinite(pending) ? _roundBlindPosition(pending) : null;
}

function _persistBlindDashboardTargetState(deviceId, pos, status = 'pending') {
  try {
    const key = `blind-state-${deviceId}`;
    const saved = _readSavedBlindState(deviceId);
    saved.targetPosition = pos;
    saved.pendingTargetPosition = pos;
    saved.pendingCommandTs = Date.now();
    saved.pendingCommandSource = 'dashboard';
    saved.lastCommandStatus = status;
    saved.isOpen = pos > 0;
    localStorage.setItem(key, JSON.stringify(saved));
  } catch (e) { /* ignore */ }
}

function _sendBlindDashboardPosition(deviceId, pos, source = 'dashboard') {
  const safePos = _roundBlindPosition(pos);
  if (typeof BlindCommandQueue !== 'undefined' && typeof BlindCommandQueue.sendPosition === 'function') {
    return BlindCommandQueue.sendPosition(deviceId, safePos, { source });
  }
  _persistBlindDashboardTargetState(deviceId, safePos, 'pending');
  const sent = typeof MQTTClient !== 'undefined' && typeof MQTTClient.publishStepperControl === 'function'
    ? MQTTClient.publishStepperControl(deviceId, { blindPosition: safePos })
    : false;
  return { sent, queued: !sent };
}

function _sendBlindDashboardStop(deviceId, emergency = false) {
  const payload = { command: emergency ? 'emergencyStop' : 'stop' };
  if (typeof BlindCommandQueue !== 'undefined' && typeof BlindCommandQueue.send === 'function') {
    // Stop is time-sensitive: try MQTT/LAN fallback, but do not replay a stale
    // stop minutes later after the blind may already be somewhere else.
    return BlindCommandQueue.send(deviceId, payload, { source: 'dashboard', persist: false });
  }
  const sent = typeof MQTTClient !== 'undefined' && typeof MQTTClient.publishStepperControl === 'function'
    ? MQTTClient.publishStepperControl(deviceId, payload)
    : false;
...
function _getBlindPositionForDevice(device) {
  ...
  if (raw === undefined || raw === null) {
    ...
    raw = saved.pendingTargetPosition ?? saved.position ?? saved.targetPosition;
  }
...
function _persistBlindDashboardState(deviceId, pos) {
  _persistBlindDashboardTargetState(deviceId, pos, 'pending');
}

function _queueBlindDashboardConfigSync(deviceId, savedState) {
  if (typeof BlindConfigSync !== 'undefined' && typeof BlindConfigSync.queue === 'function') {
    return BlindConfigSync.queue(deviceId, savedState, { source: 'dashboard' });
  }
  if (typeof BlindSchema !== 'undefined' && typeof BlindSchema.queueConfigSync === 'function') {
    return BlindSchema.queueConfigSync(deviceId, savedState);
  }

  if (typeof MQTTClient !== 'undefined' && MQTTClient.connected) {
    MQTTClient.publishConfig(deviceId, {
    ...
    <div class="device-header">
      <div class="device-info">
        ...
        <div class="device-details">
          ...
          <div class="device-status">${summary.count} blinds • ${summary.onlineCount} online</div>
        </div>
      </div>
...
function updateAllBlindsCompactCard(blindDevices = _getBlindDevices()) {
  ...
  if (status) status.textContent = `${summary.count} blinds • ${summary.onlineCount} online`;
...
function setBlindGroupPosition(rawPos) {
  ...
  blindDevices.forEach(device => {
    ...
    _persistBlindDashboardTargetState(deviceId, pos, isConnected ? 'sent' : 'queued');
    _sendBlindDashboardPosition(deviceId, pos, 'dashboard-group');
  });
...
function stopBlindGroup() {
  ...
  if (!canPublish) {
    if (typeof Toast !== 'undefined') Toast.error("Can't stop blinds right now");
    return;
  }
  ...
  blindDevices.forEach(device => {
    const deviceId = String(device.id || '').trim();
    if (!deviceId) return;
    _sendBlindDashboardStop(deviceId);
    const pos = _getBlindPositionForDevice(device);
    if (pos !== null) {
      _blindTargetLock.set(deviceId, { target: pos, timestamp: Date.now() });
  ...
  if (typeof Toast !== 'undefined') {
    if (isConnected && Toast.success) Toast.success(`Stop sent to ${blindDevices.length} blinds`);
    else if (Toast.info) Toast.info('MQTT offline - trying local stop on the LAN');
  }
...
function createBlindDeviceCard(device, state = null, index = 0) {
  ...
  const confirmedPosition = _roundBlindPosition(rawPosition);
  const pendingTarget = _getBlindPendingTarget(device.id);
  const pendingDifferent = pendingTarget !== null && Math.abs(pendingTarget - confirmedPosition) > 2;
  const position = pendingDifferent
    ? pendingTarget
    : confirmedPosition;
  const hasPendingTarget = pendingDifferent;
  const isOpen = position > 0;

  const statusText = isConnecting ? 'Connecting...' : (isOnline ? 'Online' : 'Offline');
  const posText = hasPendingTarget ? `Pending ${_badgeLabel(position)}` : _posText(position);
  ...
  card.innerHTML = `
        ...
        <div class="blind-pos-track" aria-hidden="true">
            <div class="blind-pos-fill${hasPendingTarget ? ' pending' : ''}" style="width: ${position}%"></div>
        </div>
...
function updateBlindDeviceCard(deviceId, state) {
  ...
  const needsCalibration = isOnline && (state?.isCalibrated === false || state?.calibrationMode === true);
  const savedState = _readSavedBlindState(deviceId);
  const pendingTarget = Number.isFinite(Number(savedState.pendingTargetPosition))
    ? _roundBlindPosition(Number(savedState.pendingTargetPosition))
    : null;
  const hasPendingTarget = pendingTarget !== null && Math.abs(pendingTarget - position) > 2;
  ...
  if (lock) {
    ...
    if (position === target || elapsed > 10000 || hasPendingTarget) {
  ...
  const statusText = isConnecting ? 'Connecting...' : (isOnline ? 'Online' : 'Offline');
  const posText = needsCalibration
    ? 'Finish calibration'
    : (hasPendingTarget ? `${_posText(displayPosition)} • queued ${_badgeLabel(pendingTarget)}` : _posText(displayPosition));

  // Retrieve blind type label for status text
  let blindTypeLabel = 'Roller';
  try {
    const typeLabels = { roller: 'Roller', vertical: 'Vertical', zebra: 'Zebra' };
    blindTypeLabel = typeLabels[savedState.blindType] || 'Roller';
  } catch (e) { /* ignore */ }
  ...
  const badge = card.querySelector('.blind-position-badge');
  if (badge) {
    badge.textContent = needsCalibration
      ? 'Calibrate'
      : (hasPendingTarget ? `${_badgeLabel(displayPosition)} → ${_badgeLabel(pendingTarget)}` : _badgeLabel(displayPosition));
    badge.classList.remove('open', 'closed', 'half');
    badge.classList.add(needsCalibration ? 'half' : _badgeClass(displayPosition));
  }

  // Live position bar tracks every state update (pulses while pending).
  const posFill = card.querySelector('.blind-pos-fill');
  if (posFill) {
    posFill.style.width = `${displayPosition}%`;
    posFill.classList.toggle('pending', hasPendingTarget || needsCalibration);
  }
...
async function setupFirebaseSubscription(list, emptyState, countEl) {
  try {
    ...
    // Recover any device registrations that failed during setup (offline /
    // auth race) — without this, the merge below would drop them from the
    // dashboard once their 5-minute local grace period expires.
    DeviceService.flushPendingRegistrations(homeId).catch(() => {});
    ...
    deviceSubscription = await DeviceService.subscribeToDevices(homeId, (rawDevices) => {
      ...
      // Devices whose cloud registration is still queued (failed at setup
      // time, retried on app load) must survive the merge indefinitely — they
      // are real blinds the user owns, just not in Firestore yet.
      let pendingRegIds = new Set();
      try {
        const pendingReg = JSON.parse(localStorage.getItem('zaylo-pending-device-reg') || '[]');
        if (Array.isArray(pendingReg)) {
          pendingRegIds = new Set(pendingReg.map(d => d && d.id).filter(Boolean));
        }
      } catch (e) { /* corrupted queue — fall back to the time-based grace */ }
      ...
      const recentLocalOnly = localDevices.filter(d => {
        ...
        if (pendingRegIds.has(d.id)) return true; // awaiting cloud registration — never drop
...
function handleCompactDeviceTap(card, deviceId) {
  ...
  if (isBlindOrStepper) {
    ...
    _persistBlindDashboardTargetState(deviceId, pos, (typeof MQTTClient !== 'undefined' && MQTTClient.connected) ? 'sent' : 'queued');

    if (typeof MQTTClient !== 'undefined') {
      if (type === 'stepper' || type === 'blind') {
        _sendBlindDashboardPosition(deviceId, pos, 'dashboard-compact');
      } else {
  ...
  const status = card.querySelector('.device-status');
  if (status) status.textContent = `Online • ${newState ? 'On' : 'Off'}`;
  const chip = card.querySelector('.compact-action-chip');
  ...
  list.addEventListener('click', (e) => {
    ...
    if (e.target.closest('[data-action="blindOpen"]') ||
      ...
      if (statusEl) {
        ...
        statusEl.textContent = `${connText} • ${motionText} • ${typeText}`;
      }
      ...
      const type = card.dataset.deviceType;
      if (type === 'stepper' || type === 'blind') {
        _sendBlindDashboardPosition(deviceId, pos, 'dashboard-card');
      } else {
        MQTTClient.publishControl(deviceId, { blindPosition: pos, blindOpen: pos > 0 });
      }
      if (!isConn && typeof Toast !== 'undefined' && Toast.info) {
        Toast.info('Offline — change queued, will apply when reconnected');
      }

      _persistBlindDashboardTargetState(deviceId, pos, isConn ? 'sent' : 'queued');
      updateBlindGroupControl();
      return;
    }
    ...
    if (e.target.closest('[data-action="blindAutoRule"]')) {
      ...
      try {
        ...
        const syncResult = _queueBlindDashboardConfigSync(deviceId, saved);
        if (typeof MQTTClient !== 'undefined' && !MQTTClient.connected && typeof Toast !== 'undefined' && Toast.info) {
          Toast.info('Offline — automation change will sync when reconnected');
        } else if (syncResult && syncResult.sent === false && typeof Toast !== 'undefined' && Toast.info) {
          Toast.info('Automation change queued for sync');
        }
  ...
  const { modal, close } = Modal.create({
    ...
    content: `
            <p style="color: var(--text-secondary); margin-bottom: 20px;">
                Enter the Device ID shown on your device
            </p>
            <div style="margin-bottom: 16px;">
                <input 
                    type="text" 
                    id="deviceIdInput"
                    placeholder="A1B2C3 or A1B2C3D4E5F6" 
                    maxlength="12"
                    style="
                        width: 100%;
                        padding: 16px;
    ...
    actions: [
      { label: 'Cancel', primary: false },
      {
        ...
        onClick: async () => {
          ...
          if (!/^[A-F0-9]{6}$|^[A-F0-9]{12}$/.test(id)) {
            Toast.error('Please enter a valid 6 or 12 character hex ID');
            return false;
          }
...
async function initMQTT() {
  ...
  MQTTClient.on('onConnect', async () => {
    ...
    if (typeof BlindSync !== 'undefined' && typeof BlindSync.init === 'function') {
      BlindSync.init({ deviceIds: deviceList.map(d => d.id) });
    } else if (typeof BlindSchema !== 'undefined' && typeof BlindSchema.flushPendingConfigSyncs === 'function') {
      BlindSchema.flushPendingConfigSyncs(deviceList.map(d => d.id));
    }
        ...
        if (MQTTClient.connected) {
          ...
          setTimeout(() => {
            ...
            if (!currentState || currentState._online === undefined || (!hasRealState && currentState._online === false)) {
                console.log(`[Index] Waiting for live state from ${device.id}.`);
                const waitingState = currentState
                  ? { ...currentState, _online: undefined, _waitingForLiveState: true }
                  : { _online: undefined, _waitingForLiveState: true };
                MQTTClient.deviceStates.set(device.id, waitingState);
                if (typeof StateStore !== 'undefined') StateStore.update(device.id, waitingState);
                MQTTClient.callbacks.onStateUpdate.forEach(cb => { try { cb(device.id, waitingState); } catch(e){} });
            }
          }, 3000);
          ...
          setTimeout(() => {
            const currentState = MQTTClient.getDeviceState(device.id);
            const hasRealState = currentState && (currentState.position !== undefined || currentState.blindPosition !== undefined || currentState.light !== undefined || currentState.mode !== undefined);
            if (!currentState || (!hasRealState && currentState._online !== true)) {
                console.log(`[Index] Device timeout: ${device.id}. Marking as Offline after grace period.`);
                const offlineState = currentState
                  ? { ...currentState, _online: false, _waitingForLiveState: false, _offlineConfirmed: true }
      ...
      _stateUpdateRafId = requestAnimationFrame(() => {
        _pendingStateUpdates.forEach((pendingState, pendingId) => {
          if (typeof BlindSync !== 'undefined' && typeof BlindSync.handleState === 'function') {
            BlindSync.handleState(pendingId, pendingState);
          } else if (typeof BlindSchema !== 'undefined' && typeof BlindSchema.handleConfigAck === 'function') {
            BlindSchema.handleConfigAck(pendingId, pendingState);
          }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('settingsBtn')?.addEventListener('click', () => {
    const defaultBrokerHost = (typeof MQTTClient !== 'undefined' && MQTTClient.config && MQTTClient.config.broker)
      ? MQTTClient.config.broker
      : (window.location.hostname || 'localhost');
    const currentIP = localStorage.getItem('zaylo-BrokerIP') || defaultBrokerHost;
      ...
      content: `
        ...
        <div style="margin-bottom: 24px;">
          ...
          <div class="settings-card">
            ...
            <button id="compactReorderStartBtn" class="widget-reorder-button" type="button">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="5" r="1"/><circle cx="9" cy="12" r="1"/><circle cx="9" cy="19" r="1"/><circle cx="15" cy="5" r="1"/><circle cx="15" cy="12" r="1"/><circle cx="15" cy="19" r="1"/></svg>
              <span>${currentWidgetLayout === 'large' ? 'Hold to reorder widgets' : 'Reorder Widgets'}</span>
            </button>
          </div>
        </div>
        ...
        <div style="margin-bottom: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              <label style="display:block; color:var(--text-secondary); margin-bottom:8px; font-size:14px;">MQTT Broker Host</label>
              <input type="text" id="brokerIpInput" value="${Utils.escapeHtml(currentIP)}" style="width:100%; padding:12px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:monospace; font-size:12px;">
              <p style="font-size:12px; color:var(--text-tertiary); margin-top:6px;">Default: ${Utils.escapeHtml(defaultBrokerHost)}</p>
            </div>
            <div style="margin-bottom: 16px;">
              <label style="display:block; color:var(--text-secondary); margin-bottom:8px; font-size:14px;">WSS Port</label>
              <input type="number" id="brokerPortInput" value="${Utils.escapeHtml(currentPort)}" style="width:100%; padding:12px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:monospace;">
              <p style="font-size:12px; color:var(--text-tertiary); margin-top:6px;">Default: 443 (Secure WebSockets)</p>
            </div>
            <div style="margin-bottom: 16px;">
              <label style="display:block; color:var(--text-secondary); margin-bottom:8px; font-size:14px;">WebSocket Path</label>
              <input type="text" id="brokerPathInput" value="${Utils.escapeHtml(currentPath)}" style="width:100%; padding:12px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:monospace;">
    ...
    modal.querySelectorAll('[data-widget-layout-choice]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (reorderBtn) {
          const label = reorderBtn.querySelector('span');
          if (label) label.textContent = layout === 'large' ? 'Hold to reorder widgets' : 'Reorder Widgets';
        }
        if (typeof Haptic !== 'undefined') Haptic.selection();
      });
    });
    ...
    modal.querySelector('#compactReorderStartBtn')?.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (WidgetLayout.isCompact()) {
        WidgetLayout.setCompactReorderMode(true);
      } else {
        WidgetLayout.showLargeReorderHint();
      }
      if (typeof Haptic !== 'undefined') Haptic.medium();
      if (typeof close === 'function') close();
    });
    ...
    if (storedLat && storedLon) {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${storedLat}&lon=${storedLon}&appid=${OPENWEATHER_API_KEY}`)
            .then(r => r.json())
            .then(data => {
    ...
    const applyGlobalLocation = (lat, lon, cityName) => {
        GlobalLocation.apply(lat, lon, cityName);
    };

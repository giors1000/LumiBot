...
(function () {
  ...
  const ConfigPush = {
    ...
    handleState(id, state) {
      ...
      if (echoed !== undefined && Number(echoed) >= item.rev) {
        delete p[id]; this.save(p);
      } else if ((item.attempts || 0) >= 8 && state && (state.position !== undefined || state.blindPosition !== undefined)) {
        // Older firmware without cfgRev echo — a full state proves delivery.
        delete p[id]; this.save(p);
      }
    }
  };
  ...
  const Master = {
    ...
    init() {
      ...
      const up = e => {
        ...
        applyPositionToDevices(blindIds(), this.value, `Set to ${this.value}%`);
      };
      this.track.addEventListener('pointerdown', e => {
        this.dragging = true; this.setVisual(pointer(e)); haptic('light');
    ...
    setVisual(v) {
      ...
      // Keep the 30px knob fully inside the track at both extremes.
      if (this.handle) this.handle.style.left = 'calc(15px + (100% - 30px) * ' + (this.value / 100) + ')';
  ...
  function initTabs() {
    ...
    function activate(name) {
      ...
      document.querySelectorAll('.gp-panel').forEach(p => p.classList.toggle('active', p.id === `panel-${name}`));
      try { localStorage.setItem('zaylo-spaces-tab', name); } catch (e) {}
    }
    buttons.forEach(b => b.addEventListener('click', () => { haptic('selection'); activate(b.dataset.tab); }));
    let initial = 'scenes';
  ...
  function openAutoEditor(existing) {
    ...
    function rebuild() {
      const meta = AUTO_TYPES[type];
      document.getElementById('atDynamic').innerHTML = triggerSection() + targetSection();
      document.getElementById('atBlurb').textContent = meta.blurb;
      wireDynamic();
    }
    ...
    foot.push({
      cls: 'primary', label: existing ? 'Save' : 'Create', onClick: () => {
        ...
        pushAutomationsToDevices(Array.from(new Set(prevIds.concat(ids))));
        haptic('success');
  ...
  function confirmDelete(kind, id, name) {
    ...
    const run = () => {
      if (kind === 'scene') { SceneStore.remove(id); renderScenes(); }
        ...
        if (a) pushAutomationsToDevices(a.deviceIds || []); // clears the rule on its devices
        renderAutomations();
      }
  ...
  function wireStaticEvents() {
    ...
    autoList.addEventListener('change', e => {
      ...
      if (a) {
        ...
        pushAutomationsToDevices(a.deviceIds || []);

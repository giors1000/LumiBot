...
const MQTTClient = {
    ...
    config: {
        ...
        get broker() { return localStorage.getItem('LumiBot-BrokerIP') || '192.168.0.137'; },
        get port() { return parseInt(localStorage.getItem('LumiBot-BrokerPort')) || 9001; },
        // WebSocket path - Mosquitto typically uses '/mqtt' or '' (empty)
        // Try '/mqtt' first, fall back to '/' if that fails
    ...
    async connect(options = {}) {
        ...
        // Truly Unique ID - Do NOT persist, to allow multiple tabs
        // SWM-RunID-Random
        const runId = Math.random().toString(36).substring(2, 6).toUpperCase();
        this.clientId = `SWM-Web-${runId}-${Date.now().toString().slice(-6)}`;
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                const opts = {
                    ...
                    onSuccess: () => {
                        console.log(`[MQTT] ✅ Connected successfully using MQTT v${this.protocolVersion === 4 ? '3.1.1' : '3.1'}`);
    ...
    _onConnectionLost(response) {
        ...
        if (response.errorCode === 8) {
            ...
            // 1. Wrong WebSocket path (/mqtt vs / vs empty)
            // 2. Mosquitto not configured for WebSockets
            // 3. Wrong port (9001 vs 8080 etc)
            console.error('[MQTT] Socket closed (Code 8). Possible causes:');
            console.error('  1. Broker not configured for WebSockets on port', this.config.port);
            console.error('  2. Wrong WebSocket path (current:', this.config.wsPath + ')');
            console.error('  3. Check Mosquitto config has: listener 9001 + protocol websockets');

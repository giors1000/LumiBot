...
const MQTTClient = {
                    ...
                    onSuccess: () => {
                        ...
                        setTimeout(() => {
                            ...
                            if (this._disconnectTimer) {
                                ...
                                console.log('[MQTT] ⚡ Suppressed disconnect notification (quick reconnect)');
        ...
        if (response.errorCode === 8) {
            ...
            if (!this._pathCycleComplete) {
                if (currentIndex === pathCycle.length - 1) {
                    ...
                    console.log('[MQTT] ⚠️ Tried all path options, will stop cycling');
                } else {
                    console.log(`[MQTT] Trying path fallback: "${nextPath}" instead of "${currentPath}"`);
    ...
    _performSubscription(topics) {
        ...
        if (this._activeSubscriptions.has(subKey)) {
            console.log(`[MQTT] Skipping duplicate subscription: ${subKey}`);
            return;
        }
        ...
        try {
            ...
            console.log(`[MQTT] Subscribed to: ${topics.state}, ${topics.availability}`);
        } catch (e) {
     ...
     */
    forceReconnect() {
        console.log('[MQTT] 🔄 Force reconnect requested (PWA resume or stale connection)');
        ...
        setTimeout(() => {
            console.log('[MQTT] Starting fresh connection...');
            this.connect(this.config).catch(err => {
                console.error('[MQTT] Force reconnect failed:', err);
            });
        }, 100);
    },
    ...
    handleVisibilityChange() {
        if (document.visibilityState === 'visible') {
            console.log('[MQTT] 👁️ Page became visible - checking connection health');
            ...
            if (!this.connected) {
                console.log('[MQTT] Not connected - initiating reconnect');
                this.forceReconnect();
            } else if (!this.checkConnectionHealth()) {
                console.log('[MQTT] Connection appears stale - forcing reconnect');
                this.forceReconnect();
            } else {
                console.log('[MQTT] Connection appears healthy');
    ...
    initVisibilityHandler() {
        ...
        window.addEventListener('focus', () => {
            ...
            setTimeout(() => {
                if (document.visibilityState === 'visible') {
                    console.log('[MQTT] 📱 Window focused - verifying connection');
        ...
        window.addEventListener('online', () => {
            console.log('[MQTT] 🌐 Network came online - reconnecting');
            this.forceReconnect();
        });

        this._visibilityInitialized = true;
        console.log('[MQTT] ✅ Visibility and network handlers initialized');
    },
    ...
    _onMessageArrived(message) {
        ...
        if (type === 'state') {
            ...
            if (state._online !== isOnline) {
                console.log(`[MQTT] ⚠️ Device ${deviceId} is now ${isOnline ? 'ONLINE' : 'OFFLINE'} (LWT/Availability)`);
            }

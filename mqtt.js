...
const MQTTClient = {
     ...
     *          US Eastern summer  → gmtOffset=-300, daylightOffset=60
     */
    getTimezonePayload() {
        const now = new Date();
        ...
        const gmtOffsetMinutes = -(standardOffsetMinutes);
        const daylightOffsetMinutes = dstShiftMinutes; // 0 when not in DST, typically 60 when in DST

        return { gmtOffset: gmtOffsetMinutes, daylightOffset: daylightOffsetMinutes };
    },
    ...
    syncTimezoneToAllDevices(devices) {
        ...
        if (window.DEBUG) {
            console.debug(`[MQTT] 🕐 Timezone synced to ${devices.length} device(s): GMT${tz.gmtOffset >= 0 ? '+' : ''}${tz.gmtOffset}min, DST=${tz.daylightOffset}min`);
        }
    }
};

...
const MQTTClient = {
    ...
    // Production deployments should serve the MQTT websocket endpoint from the
    // app host or set these values through localStorage during fleet rollout.
    //
    ...
    publishStepperControl(deviceId, payload, options = {}) {
        return this._publish(`lumibot/${deviceId.toUpperCase()}/stepper/set_position`, payload, options);
    },
    ...
    _triggerLocalHttpFallback(topic, payload) {
        ...
        if (stepperMatch) {
            ...
            fallbackPayload = this._normalizeStepperLocalPayload(payload);
            if (!fallbackPayload) {
        ...
        .then(async res => {
            ...
            if (res.ok) {
                ...
                if (res.status === 409 && stepperMatch) {
                    this.pendingMessages.delete(topic);
                }
                const failure = {
                    deviceId,

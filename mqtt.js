...
const MQTTClient = {
    ...
    /**
     * Calculate the correct timezone payload for the current browser locale.
     * Uses the browser's getTimezoneOffset() which ALREADY includes DST.
     *
     * Returns { gmtOffset, daylightOffset } in MINUTES, matching the firmware's
     * expected format in processConfigCommand().
     *
     * Strategy (backward-compatible two-field approach):
     *   - gmtOffset  = the standard (winter) offset for this timezone
     *   - daylightOffset = the additional DST shift currently in effect (0 or 60 typically)
     *   - Firmware adds them: total = gmtOffset + daylightOffset → correct local time
     *
     * Example: UK in summer (BST) → gmtOffset=0, daylightOffset=60
     *          UK in winter (GMT) → gmtOffset=0, daylightOffset=0
     *          US Eastern summer  → gmtOffset=-18000, daylightOffset=3600
     */
    getTimezonePayload() {
        const now = new Date();
        ...
        // Standard offset is the LARGER getTimezoneOffset (remember: getTimezoneOffset
        // returns the difference UTC−local in minutes, so winter is more positive/less negative)
        const standardOffsetMinutes = Math.max(jan.getTimezoneOffset(), jul.getTimezoneOffset());

        // Current offset from UTC (includes DST if active)
        const currentOffsetMinutes = now.getTimezoneOffset();

        // DST shift = difference between standard and current offset
        const dstShiftMinutes = standardOffsetMinutes - currentOffsetMinutes;

        // Convert to "east of UTC" convention (getTimezoneOffset is west-positive)
        // CRITICAL: ESP32's configTime() expects offsets in SECONDS, not minutes.
        // The Weather API also returns timezone offsets in seconds. Using seconds
        // here ensures both sources are consistent and configTime() works correctly.
        const gmtOffsetSeconds = -(standardOffsetMinutes) * 60;
        const daylightOffsetSeconds = dstShiftMinutes * 60; // 0 when not in DST, typically 3600 when in DST

        return { gmtOffset: gmtOffsetSeconds, daylightOffset: daylightOffsetSeconds };
    },
    ...
    syncTimezoneToDevice(deviceId) {
        ...
        return this.publishConfig(deviceId, {
            config: {
    ...
    syncTimezoneToAllDevices(devices) {
        ...
        if (window.DEBUG) {
            console.debug(`[MQTT] 🕐 Timezone synced to ${devices.length} device(s): GMT${tz.gmtOffset >= 0 ? '+' : ''}${tz.gmtOffset}s, DST=${tz.daylightOffset}s`);
        }
    }
};

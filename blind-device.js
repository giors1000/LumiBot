...
function setupControls() {
    ...
    // Live Stop control: a short tap sends a graceful stop ({command:'stop'});
    // pressing and holding for 600ms escalates to the firmware's emergency stop
    // ({command:'emergencyStop'} — immediate halt, motor released). Both route to
    // the device's processControlCommand handler.
    ...
    if (stopBtn) {
        ...
        const sendStop = (emergency) => {
            ...
            if (typeof Toast !== 'undefined') Toast.success(emergency ? 'Emergency stop' : 'Stopping…');
            if (typeof addLogEntry === 'function') addLogEntry(emergency ? '🛑' : '✋', emergency ? 'Emergency stop triggered' : 'Stop');
        };

        stopBtn.addEventListener('pointerdown', (e) => {
            e.preventDefault();
            emergencyFired = false;
            stopBtn.style.transform = 'scale(0.96)';
...
function setStopButtonVisible(visible) {
    ...
    if (visible) {
        const hint = document.getElementById('stopBtnHint');
        if (hint) hint.textContent = 'Hold for emergency stop';
    }
}

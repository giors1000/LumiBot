...
function handleStateUpdate(state) {
    ...
    if (pos !== undefined && pos !== BlindState.position && !BlindState.isDragging) {
        ...
        // Snap the visuals to the true remote position
        // This instantly corrects drift and handles external hardware stops
        ...
        // Force the animation loops to revive and paint the snapped position
    ...
    if (state.isMoving !== undefined) {
        ...
        // Otherwise, if the user stopped it via physical touch sensor mid-travel,
        // the UI's targetPosition would still be 100%, and the local animation
        // engine would immediately start moving the UI blind again!
...
function _calculateAnimationSpeed(diff) {
    ...
    if (!BlindState.config || !BlindState.config.stepperTop || !BlindState.config.stepperBottom) {

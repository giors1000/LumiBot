...
function restoreSettingsGroupStates() {
    ...
    groupIds.forEach(groupId => {
        ...
        // Calibration defaults to open if limits setup is needed.
        // Others default to closed (collapsed).
        let defaultState = false;
        if (groupId === 'group-calibration') {
            const isUncalibrated = (BlindState.isCalibrated === false);
            if (isUncalibrated) {
                defaultState = true;
            }
        }
...
function updateSettingsGroupSummaries() {
    ...
    if (networkBadge) {
        ...
        if (isOnline) {
            ...
            networkBadge.textContent = ssid ? ssid : 'Connected';
        } else {

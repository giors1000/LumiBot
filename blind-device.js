...
function handleStateUpdate(state) {
    ...
    if (pos !== undefined && pos !== BlindState.position && !BlindState.isDragging) {
        ...
        // Kick off smooth animation loops — they will interpolate _visualPos
        // and _displayPos toward the new BlindState.position at the physical
        // motor speed, so the UI matches the real blind 1:1.
...
function updateCalibrationWarning() {
    ...
    if (!BlindState.isCalibrated && BlindState.blindType === 'stepper') {
...
function updatePresenceBadge(state) {
    ...
    // Only show if a linked device exists
    if (!BlindState.linkedDeviceId) {
    ...
    const linkedState = typeof MQTTClient !== 'undefined'
        ? MQTTClient.getDeviceState(BlindState.linkedDeviceId) : null;

    if (state && state.linkedDeviceOnline === false) {
        badge.className = 'presence-badge';
        if (text) text.textContent = 'Switch Offline';
        return;
    }

    // Check presence from linked device's motion/still state
    const hasMotion = linkedState && (linkedState.motion === true || linkedState.still === false);
    const isEmpty = linkedState && linkedState.motion === false && linkedState.still === true;

    if (hasMotion) {
        badge.className = 'presence-badge occupied';
        if (text) text.textContent = 'Occupied';
    } else if (isEmpty) {
        badge.className = 'presence-badge empty';
...
// Track presence subscription so we can re-subscribe when linked device changes
let _presenceUnsubscribe = null;

function setupPresenceSubscription() {
    // Unsubscribe from previous linked device if any
    if (_presenceUnsubscribe) {
        _presenceUnsubscribe();
        _presenceUnsubscribe = null;
    }
    if (typeof StateStore === 'undefined' || !BlindState.linkedDeviceId) return;

    _presenceUnsubscribe = StateStore.subscribe(BlindState.linkedDeviceId, () => {
        updatePresenceBadge(StateStore.get(BlindState.deviceId));
    });
}

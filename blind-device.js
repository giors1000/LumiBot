    ...
    if (typeof Auth !== 'undefined' && typeof DeviceService !== 'undefined') {
        const syncFirebase = async () => {
            ...
            if (user) {
                ...
                const fbDevice = await DeviceService.getDevice(user.uid, BlindState.deviceId);
                if (fbDevice) {
                    let updated = false;
    ...
    document.getElementById('removeDeviceBtn')?.addEventListener('click', () => {
        ...
        Modal.confirm(
            ...
            async () => {
                ...
                localStorage.setItem('zaylo-devices', JSON.stringify(updated));
...
function updateConfigUI() {
    ...
    if (sunsetEl) {
        ...
        sunsetEl.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> Sunset ${offsetStr} min${timeStr}`;
    }
    ...
    if (typeof Auth !== 'undefined' && typeof DeviceService !== 'undefined') {
        ...
        if (user) {
            DeviceService.init().then(() => {
                DeviceService.updateDevice(user.uid, BlindState.deviceId, stateObj);

                ...
                if (MQTTClient.connected) {
                    ...
                    setTimeout(() => {
                        ...
                        if (!state || !hasRealState || state._online === undefined || state._online === false) {
                            console.log(`[BlindDevice] Device timeout: ${BlindState.deviceId}. Assuming Online since blind firmware may lack getState.`);
                            const assumedState = state ? { ...state, _online: true } : { _online: true };
                            MQTTClient.deviceStates.set(BlindState.deviceId, assumedState);
                            if (typeof StateStore !== 'undefined') StateStore.update(BlindState.deviceId, assumedState);
                        }
                    }, 3000);
                }
            }, 300);
...
function handleStateUpdate(state) {
    ...
    if (state.config) {
        // Firmware sends morningDays as boolean[] but the UI modal expects object[]
        // Convert boolean[] to object[] before merging, preserving existing per-day settings
        if (state.config.morningDays && Array.isArray(state.config.morningDays) &&
            state.config.morningDays.length === 7 && typeof state.config.morningDays[0] === 'boolean') {
...
/**
 * Continuously smoothly animate the blinds visualization toward the actual target position.
 * Uses a time-scaled lerp (exponential smoothing) inside requestAnimationFrame so that
 * continuous MQTT position updates produce fluid, natural motion without restarting the curve.
 */
function animateVisualization() {
    if (_vizAnimFrameId) return; // already tick loop running

    let lastTime = performance.now();

    function tick(now) {
        const dt = Math.min(now - lastTime, 50); // cap delta time to 50ms at most
        lastTime = now;

        const target = BlindState.position;
        const diff = target - BlindState._visualPos;

        // If we are close enough, snap to position and stop loop
        ...
        // Exponential smoothing, adjusting for frame time to maintain consistent speed
        // roughly equivalent to lerping by 0.1 at 60fps
        const lerpFactor = 1 - Math.exp(-0.008 * dt);
        BlindState._visualPos = BlindState._visualPos + diff * lerpFactor;
...
function animatePositionLabel() {
    ...
    function tick(now) {
        ...
        const lerpFactor = 1 - Math.exp(-0.01 * dt);
        BlindState._displayPos = BlindState._displayPos + diff * lerpFactor;
...
function _publishRulesConfig() {
    ...
    if (typeof MQTTClient !== 'undefined' && MQTTClient.connected) {
        ...
        // MQTT payload values
        const gmtOffsetSeconds = -(standardTimezoneOffset * 60);
        const daylightOffsetSeconds = isDST ? 3600 : 0;
        ...
        MQTTClient.publishConfig(BlindState.deviceId, {
            ...
            config: {
                ...
                morningDays: BlindState.config.morningDays ? BlindState.config.morningDays.map(d => d.enabled) : null,
                ...
                gmtOffset: Math.floor(gmtOffsetSeconds / 60), // send in minutes
                daylightOffset: Math.floor(daylightOffsetSeconds / 60) // send in minutes
            }
        });
    }
}
...
function showSunsetConfigModal() {
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            ...
            <div style="margin-top:20px; padding:12px; border-radius:12px; background:rgba(99,102,241,0.1); border:1px solid rgba(99,102,241,0.2); text-align:center;">
                <p style="color:var(--accent); font-size:12px; line-height:1.4; margin:0;">🌍 Geographic location is now managed globally in the main app Settings.</p>
            </div>
        `,
        actions: [
...
function _getNextMorningWakeupMessage() {
    ...
    for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
        ...
        // If offset is 0 (today) and the time has already passed, 
...
function updateUI() {
    ...
    // Animated position counter (follows real position with smooth loop)
...
function getDeviceName() {
    ...
    const devices = JSON.parse(localStorage.getItem('zaylo-devices') || '[]');
...
function setDeviceName(name) {
    ...
    if (typeof DeviceList !== 'undefined') {
        ...
        if (device) {
            device.name = name;
            localStorage.setItem('zaylo-devices', JSON.stringify(devices));
        }
    }

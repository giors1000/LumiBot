...
function setupLocalControlFeedback() {
    ...
    window.addEventListener('zaylo:local-control-failed', (event) => {
        ...
        const responseMessage = detail.response && detail.response.message
            ...
            : '';
        const isCalibrationReject = detail.status === 409 || /calibration required/i.test(responseMessage);
        if (!isCalibrationReject) return;
        ...
        if (typeof setStopButtonVisible === 'function') setStopButtonVisible(false);
        removeRejectedPendingPosition(detail.payload);
...
function setupControls() {
    ...
    if (stopBtn) {
        ...
        const sendStop = (emergency) => {
            ...
            if (typeof MQTTClient === 'undefined' || !MQTTClient.connected) {
                if (typeof Toast !== 'undefined') Toast.error(emergency ? "Can't emergency stop while offline" : "Can't stop while offline");
...
function showSunsetConfigModal() {
    // Sunset offset is now PER-DEVICE (overrides the home-wide default). The slider
    // defaults to the device's own override if set, otherwise the global default.
    ...
    const hasOverride = BlindState.config.sunsetOffset !== null &&
        BlindState.config.sunsetOffset !== undefined && Number.isFinite(perDevice);
    const effectiveOffset = hasOverride ? perDevice : globalOffset;
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            <div class="setting-item" style="padding: 12px 0 8px 0; border: none;">
                <div class="setting-left">
                    <span class="setting-label">Sunset Offset</span>
                    <span class="setting-sublabel">This blind only · negative = before sunset, positive = after</span>
                </div>
            </div>
            ...
            <div style="margin-top:10px; padding:10px; border-radius:10px; background:rgba(20,184,166,0.07); border:1px solid rgba(20,184,166,0.18);">
                <p style="color:var(--accent); font-size:11px; line-height:1.4; margin:0; display:flex; align-items:flex-start; gap:6px;">
                    ...
                    <span>Sets the offset for <strong>this blind only</strong>. The home-wide default is still set from the Home Page Settings and applies to blinds without their own override.</span>
                </p>
            </div>
        ...
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: 'Save', primary: true, onClick: () => {
                    ...
                    // Store as a per-device override (a finite number, including 0).
    ...
    offsetInput.addEventListener('input', (e) => {
        offsetDisplayEl.textContent = fmtOffset(parseInt(e.target.value, 10));
        if (typeof Haptic !== 'undefined') Haptic.light();
    });
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            ...
            <div id="tempReopenFields" style="display:${reopenEnabled ? 'block' : 'none'};">
                <div class="setting-item" style="padding: 12px 0; border: none;">
                    <div class="setting-left">
                        <span class="setting-label">Reopen Below (°C)</span>
                        <span class="setting-sublabel">Kept below the close threshold to avoid chatter</span>
                    </div>
                </div>
                <input type="number" id="tempReopenThresholdInput" value="${reopenThreshold}" class="modal-input" placeholder="Reopen °C">
        ...
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: 'Save', primary: true, onClick: () => {
                    ...
                    if (isNaN(reopenThreshVal)) reopenThreshVal = Math.max(0, BlindState.config.tempThreshold - 5);
                    // Enforce the deadband: the reopen threshold must stay strictly
                    // below the close threshold (mirrors the firmware clamp).
    ...
    try {
        ...
        const waitForCalibrationEcho = (cmd, validator, timeoutMs = 6000) => {
            return new Promise(resolve => {
                ...
                if (typeof MQTTClient !== 'undefined') {
                    ...
                    MQTTClient.publishConfig(BlindState.deviceId, { cmd });
                    setTimeout(() => {
                        if (MQTTClient.connected) {
                            MQTTClient.publishControl(BlindState.deviceId, { command: 'getState' });
                        }
        ...
        cleanActionBtn.addEventListener('click', async () => {
            if (currentStep === 1) {
                ...
                const result = await waitForCalibrationEcho('save_top', (snapshot, limits) => limits.top !== null);
                setActionBusy(false);
                if (!result.ok) {
                ...
                const result = await waitForCalibrationEcho('save_bottom', (snapshot, limits) => {
                    const range = (limits.top !== null && limits.bottom !== null)
                        ...
                        : 0;
                    return snapshot.isCalibrated === true && range >= 500;
                });
                setActionBusy(false);
                if (!result.ok) {
                    BlindState.isCalibrated = false;
                ...
                if (cleanActionBtn.classList.contains('success')) {
                    ...
                    MQTTClient.publishStepperControl(BlindState.deviceId, { position: 100 });
                    
                    let elapsed = 0;
                    const duration = 5000;
                    const intervalMs = 50;
                    let sentCloseCommand = false;
                    ...
                    const testRun = setInterval(() => {
                        ...
                        if (elapsed < duration / 2) {
                            ...
                            if (!sentCloseCommand) {
                                sentCloseCommand = true;
                                MQTTClient.publishStepperControl(BlindState.deviceId, { position: 0 });
                            }
...
function updateCalibrationTools() {
    ...
    const canNudge = BlindState.isOnline && !BlindState.isMoving && BlindState.isCalibrated !== false;
...
function updateConfigUI() {
    ...
    if (tempEl) {
        ...
        if (rConfig.tempReopenEnabled === true) {
            ...
            reopenStr = ` · reopens < ${rt}°C`;
        }
        tempEl.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> > ${thresh}°C${reopenStr}`;
    }
...
function saveDeviceState(_mqttPayload = null, skipDevicePublish = false) {
    ...
    // Publish to MQTT via the reliable, acked, offline-queued full sync path.
    // The first parameter is retained for older callers, but targeted one-off
    // config publishes are intentionally avoided so settings have one contract.
...
function handleDiagnosticsMessage(payload) {
    ...
    BlindState.maintenance.suspectedStalls = stepper.suspectedStalls !== undefined ? Number(stepper.suspectedStalls) : BlindState.maintenance.suspectedStalls;
...
function renderMaintenanceInsights() {
    ...
    const stalls = BlindState.maintenance.suspectedStalls;
    const confidence = _numberOrNull(BlindState.calibration.confidence);
    ...
    _setHealthText('infoLastStop', BlindState.calibration.lastEmergencyStopRecorded
        ...
        : 'None');
    _setHealthText('infoStalls', stalls !== null && stalls !== undefined && Number.isFinite(Number(stalls)) ? String(stalls) : '--');
}
...
function updateUpcomingAutomations() {
    ...
    if (rules.temperature) {
        ...
        let detail = `When outdoor temp > ${thresh}\u00b0C`;
...
function removeRejectedPendingPosition(payload) {
    if (!payload || typeof payload !== 'object') return;
    const rejectedRaw = payload.position !== undefined ? payload.position : payload.blindPosition;
    const rejectedPosition = Number(rejectedRaw);
    if (!Number.isFinite(rejectedPosition)) return;

    const before = pendingCommands.length;
    pendingCommands = pendingCommands.filter(cmd => {
        const cmdRaw = cmd && (cmd.blindPosition !== undefined ? cmd.blindPosition : cmd.position);
        const cmdPosition = Number(cmdRaw);
        return !Number.isFinite(cmdPosition) || Math.round(cmdPosition) !== Math.round(rejectedPosition);
    });

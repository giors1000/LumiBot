...
function showSunsetConfigModal() {
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            ...
            <input type="number" id="sunsetOffsetInput" value="${defaultOffset}" class="blind-slider"
                style="width:100%; padding:14px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:var(--font-family); font-size:16px;">
            ...
            <div style="text-align: center; font-size: 16px; font-weight: bold; margin-top: 8px; color: var(--blind-accent);" id="sunsetTargetDisplay">${defaultTarget}%</div>
        `,
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: 'Save', primary: true, onClick: () => {
                    BlindState.config.sunsetOffset = parseInt(modal.querySelector('#sunsetOffsetInput').value, 10) || 0;
                    BlindState.config.sunsetTarget = parseInt(modal.querySelector('#sunsetTargetInput').value, 10) || 0;

                    const offsetStr = BlindState.config.sunsetOffset >= 0 ? `+${BlindState.config.sunsetOffset}` : `${BlindState.config.sunsetOffset}`;
                    document.getElementById('sunsetRuleDisplay').innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> Sunset ${offsetStr} min`;
...
function showPresenceConfigModal() {
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            ...
            <input type="number" id="motionTimeoutInput" value="${defaultTimeout}" min="1" class="blind-slider"
                style="width:100%; padding:14px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:var(--font-family); font-size:16px;">
            ...
            <div style="text-align: center; font-size: 16px; font-weight: bold; margin-top: 8px; color: var(--blind-accent);" id="presenceTargetDisplay">${defaultTarget}%</div>
        `,
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: 'Save', primary: true, onClick: () => {
                    BlindState.config.motionTimeout = Math.max(1, parseInt(modal.querySelector('#motionTimeoutInput').value, 10)) || 5;
                    BlindState.config.presenceTarget = parseInt(modal.querySelector('#presenceTargetInput').value, 10) || 0;

                    document.getElementById('presenceRuleDisplay').innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> ${BlindState.config.motionTimeout} min timeout`;
...
function showMorningConfigModal() {
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            ...
            <input type="time" id="morningTimeInput" value="${defaultTime}" class="blind-slider"
                style="width:100%; padding:14px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:var(--font-family); font-size:16px;">
            ...
            <input type="number" id="morningDurationInput" value="${defaultDuration}" min="1" max="120" class="blind-slider"
                style="width:100%; padding:14px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:var(--font-family); font-size:16px;">
            ...
            <div style="text-align: center; font-size: 16px; font-weight: bold; margin-top: 8px; color: var(--blind-accent);" id="morningTargetDisplay">${defaultTarget}%</div>
        `,
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: 'Save', primary: true, onClick: () => {
                    ...
                    BlindState.config.morningDuration = Math.max(1, parseInt(modal.querySelector('#morningDurationInput').value, 10)) || 30;
                    ...
                    document.getElementById('morningRuleTimeDisplay').textContent = `${BlindState.config.morningTime} — Gradual`;
...
function showNightLockConfigModal() {
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            ...
            <input type="time" id="nightTimeInput" value="${defaultTime}" class="blind-slider"
                style="width:100%; padding:14px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:var(--font-family); font-size:16px;">
            ...
            <div style="text-align: center; font-size: 16px; font-weight: bold; margin-top: 8px; color: var(--blind-accent);" id="nightTargetDisplay">${defaultTarget}%</div>
        `,
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: 'Save', primary: true, onClick: () => {
                    ...
                    BlindState.config.nightTarget = parseInt(modal.querySelector('#nightTargetInput').value, 10) || 0;

                    document.getElementById('nightRuleTimeDisplay').innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> ${BlindState.config.nightTime} — Daily`;
...
function showTempConfigModal() {
    ...
    const { modal, close } = Modal.create({
        ...
        content: `
            ...
            <input type="number" id="tempThresholdInput" value="${defaultTemp}" class="blind-slider"
                style="width:100%; padding:14px; background:var(--bg-glass); border:1px solid var(--border-glass); border-radius:12px; color:var(--text-primary); font-family:var(--font-family); font-size:16px;">
            ...
            <div style="text-align: center; font-size: 16px; font-weight: bold; margin-top: 8px; color: var(--blind-accent);" id="tempTargetDisplay">${defaultTarget}%</div>
        `,
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: 'Save', primary: true, onClick: () => {
                    BlindState.config.tempThreshold = parseInt(modal.querySelector('#tempThresholdInput').value, 10) || 30;
                    ...
                    document.getElementById('tempRuleDisplay').innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> > ${BlindState.config.tempThreshold}°C`;
...
function setupSettings() {
            ...
            const { modal, close } = Modal.create({
                ...
                content: `
                    <div style="text-align: center; margin-bottom: 20px;">
                        <span id="${setting}Display" style="font-size: 32px; font-weight: 800; color: var(--blind-accent);">${currentVal}°</span>
                    </div>
    ...
    document.querySelector('[data-setting="stepperSpeed"]')?.addEventListener('click', () => {
        if (typeof Modal === 'undefined') return;

        const currentVal = BlindState.config.stepperSpeed !== undefined ? BlindState.config.stepperSpeed : 2000;

        const { modal, close } = Modal.create({
            title: 'Stepper Motor Speed',
            content: `
                <div style="text-align: center; margin-bottom: 20px;">
                    <span id="stepperSpeedDisplay" style="font-size: 32px; font-weight: 800; color: var(--blind-accent);">${currentVal}</span>
                    <span style="font-size: 14px; color: var(--text-tertiary); margin-left: 4px;">steps/s</span>
                </div>
                <input type="range" id="stepperSpeedInput" min="100" max="5000" step="100" value="${currentVal}"
                    style="width: 100%; height: 8px; border-radius: 4px; -webkit-appearance: none; background: var(--bg-tertiary); outline: none;"
                    class="blind-slider">
            ...
            actions: [
                { label: 'Cancel', primary: false },
                {
                    label: 'Save', primary: true,
                    onClick: () => {
                        const input = modal.querySelector('#stepperSpeedInput');
                        const val = parseInt(input.value, 10);
                        BlindState.config.stepperSpeed = val;
                        updateConfigUI();
                        saveDeviceState();
        ...
        // Live update display
        const inputEl = modal.querySelector('#stepperSpeedInput');
        const displayEl = modal.querySelector('#stepperSpeedDisplay');
        if (inputEl && displayEl) {
            inputEl.addEventListener('input', (e) => {
                displayEl.textContent = `${e.target.value}`;
                if (typeof Haptic !== 'undefined') Haptic.light();
...
function updateConfigUI() {
    ...
    const speedVal = document.getElementById('stepperSpeedValue');
    if (speedVal) speedVal.textContent = `${BlindState.config.stepperSpeed || 2000}`;
    ...
    const stepperSpeedItem = document.getElementById('stepperSpeedSettingItem');
    ...
    if (stepperSpeedItem) stepperSpeedItem.style.display = isStepper ? 'flex' : 'none';
    ...
    if (presenceEl) {
        ...
        presenceEl.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> ${timeout} min timeout`;
    }

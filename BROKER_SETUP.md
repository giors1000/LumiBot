...
currently defaults to `ernesto-heptamerous-lourdes.ngrok-free.dev`, while
firmware takes its default broker from `StepperMote/secrets.h`. A commercial
deployment still needs a stable broker hostname you control before units ship.
...
|---|---|---|
| PWA | host / port / ws path / credentials | Default host in `mqtt.js`, or localStorage overrides `zaylo-BrokerIP`, `zaylo-BrokerPort`, `zaylo-BrokerPath`, `zaylo-BrokerUser`, `zaylo-BrokerPass` |

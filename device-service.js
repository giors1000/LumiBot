...
const DeviceService = {
    ...
    async _doInit() {
        try {
            ...
            const { getFirestore, collection, doc, getDocs, setDoc, deleteDoc, updateDoc, query, orderBy, onSnapshot } =
            ...
            if (window.DEBUG) console.log('[DeviceService] Firestore initialized');
    ...
    async subscribeToDevice(userId, deviceId, onUpdate) {
        ...
        try {
            ...
            return this._onSnapshot(deviceRef, (docSnap) => {
                if (docSnap.exists()) {
    ...
    async subscribeToDevices(userId, onUpdate) {
        ...
        try {
            ...
            let q;
            try {
                q = this._query(devicesCol, this._orderBy('addedAt', 'desc'));
            } catch (e) {
                q = devicesCol;
            }

            return this._onSnapshot(q, (snapshot) => {
                const devices = [];
    ...
    async saveDeviceOrder(userId, orderedIds) {
        ...
        try {
            const userRef = this._doc(this.db, 'users', userId);
            await this._setDoc(userRef, { deviceOrder: orderedIds }, { merge: true });

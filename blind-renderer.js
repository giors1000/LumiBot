 ...
 *
 * Animates BlindState._visualPos toward BlindState.targetPosition at the
 * physical motor speed, so the on-screen blind tracks the real device's
 * constant-velocity travel and smoothly bridges the 1 Hz MQTT position
 * snapshots without ever predicting unconfirmed future states. This is the
...
function updateVertical(position, container = null) {
    ...
    const frontEl = scope.querySelector('.vertical-bead-chain.front');
    const backEl = scope.querySelector('.vertical-bead-chain.back');
...
function updateZebra(position, container = null) {
    const scope = container || document;
    const fabricBack = scope.querySelector('.zebra-fabric-container.back');
    const fabricFront = scope.querySelector('.zebra-fabric-container.front');
    const bottomBar = scope.querySelector('.zebra-bottom-bar');
    const layer0 = scope.querySelector('.zebra-layer-0');
...
if (typeof window !== 'undefined') {
    ...
    window.addEventListener('resize', () => {
        ...
        _zebraResizeRaf = requestAnimationFrame(() => {
            _zebraResizeRaf = 0;
            _measureZebraBandHeight(document);

...
function setupSettingHandlers() {
    ...
    document.querySelector('[data-setting="city"]')?.addEventListener('click', () => {
        ...
        const content = `
            <div style="margin-bottom: var(--spacing-lg);">
                <div style="display: flex; align-items: center; gap: var(--spacing-sm); margin-bottom: var(--spacing-sm);">
                    ...
                    <div>
                        <div style="font-weight: 600; color: var(--text-primary); font-size: var(--font-size-lg);">Global Location</div>
                        <div style="font-size: var(--font-size-sm); color: var(--text-secondary);">Currently: <strong>${config.city || 'Not Set'}</strong></div>
                    </div>
                </div>
                <div style="background: rgba(99,102,241,0.1); 
                            border-radius: var(--radius-md); padding: var(--spacing-sm) var(--spacing-md); 
                            margin-top: var(--spacing-md); border: 1px solid rgba(99,102,241,0.2);
                            font-size: var(--font-size-sm); color: var(--accent); text-align: center;">
                    🌍 Geographic location is now managed globally. Please return to the <strong>Index Page</strong> and tap the gear icon to change your global location.
                </div>
            </div>
        `;
        ...
        const { backdrop, modal, close } = Modal.create({
            ...
            content,
            actions: [
                { label: 'Close', primary: true, onClick: () => { return true; } }
            ]
        });
    });

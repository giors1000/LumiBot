...
const DeviceState = {
    ...
    // Radar Heatmap Stats
    heatmapStats: {
        daily: new Array(18).fill(0), // 9 gates * (move + static)
        weekly: new Array(18).fill(0),
        dayStart: Date.now(),
        weekStart: Date.now()
    },
    currentHeatmapMode: 'live', // 'live', 'today', 'today'
    heatmapSaveTimer: null      // Debounce timer for firebase saves
};
...
async function init() {
    ...
    if (Auth.getUser() && window.DeviceService && typeof DeviceService.subscribeToDevice === 'function') {
        try {
            ...
            DeviceService.subscribeToDevice(Auth.getUser().uid, DeviceState.deviceId, (deviceData) => {
                if (deviceData) {
                    // console.log('[Device] Real-time metadata update from Firebase:', deviceData);
    ...
    // Save heatmap stats on page exit to prevent data loss
    window.addEventListener('beforeunload', () => {
        if (DeviceState.heatmapSaveTimer) {
            clearTimeout(DeviceState.heatmapSaveTimer);
            saveHeatmapStats();
        }
    });
}
...
async function connectMQTT() {
    ...
    try {
        ...
        if (!DeviceState.initialLoadComplete) {
            // console.log('[Device] MQTT race finished (connected or timed out). forcing UI');
            hideInitialLoader();
        }
...
async function loadFirebaseDataAsync() {
    try {
        ...
        // Load Heatmap Stats
        if (persistedDevice.heatmapStats) {
            console.log('[Device] Loaded historical heatmap data');
            DeviceState.heatmapStats = persistedDevice.heatmapStats;

            // Validate array lengths
            if (!DeviceState.heatmapStats.daily || DeviceState.heatmapStats.daily.length !== 18) {
                DeviceState.heatmapStats.daily = new Array(18).fill(0);
            }
            if (!DeviceState.heatmapStats.weekly || DeviceState.heatmapStats.weekly.length !== 18) {
                DeviceState.heatmapStats.weekly = new Array(18).fill(0);
            }
        }
...
function updateUI(state, incomingMode = undefined) {
    ...
    // Radar Visualizer - Heatmap
    updateRadarHeatmap(state);
}

function updateRadarHeatmap(state) {
    const heatmapContainer = document.getElementById('radarHeatmap');
    const grid = document.getElementById('heatmapGrid');
    const mode = DeviceState.currentHeatmapMode;

    // Check if we have gate data (array of 18 integers: [move0, static0, ...])
    if (!state.radarGates || !Array.isArray(state.radarGates) || state.radarGates.length < 18) {
    ...
    // ACCUMULATE DATA
    const now = Date.now();
    const stats = DeviceState.heatmapStats;

    // Reset Day if 24h passed
    if (now - stats.dayStart > 24 * 60 * 60 * 1000) {
        stats.daily = new Array(18).fill(0);
        stats.dayStart = now;
        console.log('[Device] Reset daily heatmap stats');
    }
    // Reset Week if 7d passed
    if (now - stats.weekStart > 7 * 24 * 60 * 60 * 1000) {
        stats.weekly = new Array(18).fill(0);
        stats.weekStart = now;
        console.log('[Device] Reset weekly heatmap stats');
    }

    // Accumulate current values
    for (let k = 0; k < 18; k++) {
        stats.daily[k] += state.radarGates[k];
        stats.weekly[k] += state.radarGates[k];
    }

    // Persist stats periodically (throttled to 60s)
    if (!DeviceState.heatmapSaveTimer) {
        DeviceState.heatmapSaveTimer = setTimeout(saveHeatmapStats, 60000);
    }

    // RENDER
    // First time init: Create bars if empty
    if (grid.children.length === 0) {
        for (let i = 0; i < 9; i++) {
            ...
            moveBar.className = 'heatmap-bar move';

            const staticBar = document.createElement('div');
            staticBar.className = 'heatmap-bar static';

            barContainer.appendChild(staticBar); // Static is background
            barContainer.appendChild(moveBar);   // Move is foreground
            grid.appendChild(barContainer);
        }
    }

    // Determine values to show based on mode
    let displayValues = [];
    let maxVal = 100; // Default for live

    if (mode === 'live') {
        displayValues = state.radarGates;
        maxVal = 100;
    } else if (mode === 'today') {
        displayValues = stats.daily;
        // Find max in daily for auto-scaling
        const currentMax = Math.max(...displayValues);
        maxVal = currentMax > 100 ? currentMax : 100;
    } else if (mode === 'week') {
        displayValues = stats.weekly;
        const currentMax = Math.max(...displayValues);
        maxVal = currentMax > 100 ? currentMax : 100;
    }
    ...
    for (let i = 0; i < 9; i++) {
        ...
        const moveVal = displayValues[i * 2];
        const staticVal = displayValues[i * 2 + 1];

        const moveBar = bars[i].querySelector('.heatmap-bar.move');
        const staticBar = bars[i].querySelector('.heatmap-bar.static');

        // Scale 0-100% relative to maxVal
        const movePct = (moveVal / maxVal) * 100;
        const staticPct = (staticVal / maxVal) * 100;

        if (moveBar) moveBar.style.height = `${Math.min(100, movePct)}%`;
        if (staticBar) staticBar.style.height = `${Math.min(100, staticPct)}%`;
    }
}

// Global function for buttons
window.setHeatmapMode = function (mode) {
    if (DeviceState.currentHeatmapMode === mode) return;

    DeviceState.currentHeatmapMode = mode;
    console.log('[Device] Heatmap mode set to:', mode);
    ...
    // Force immediate re-render if we have state
    if (DeviceState.state) {
        updateRadarHeatmap(DeviceState.state);
    }
};

async function saveHeatmapStats() {
    DeviceState.heatmapSaveTimer = null;
    if (!Auth.getUser() || !DeviceState.deviceId) return;

    try {
        console.log('[Device] Saving heatmap stats to Firebase...');
        await DeviceService.updateDevice(Auth.getUser().uid, DeviceState.deviceId, {
            heatmapStats: DeviceState.heatmapStats
        });
    } catch (e) {
        console.warn('[Device] Failed to save heatmap stats:', e);
    }
}
...
function syncConfigUI(config) {
    ...
    // DEBUG: Log all received config keys to help diagnose missing settings
    console.log('[Device] syncConfigUI keys:', Object.keys(config));

    // Check for specific missing keys to alert developer/user
    if (config.presenceDisplayTimeout === undefined) console.warn('[Device] ⚠️ syncConfigUI: presenceDisplayTimeout is MISSING from config (Possible JSON truncation?)');
    else console.log(`[Device] syncConfigUI: presenceDisplayTimeout = ${config.presenceDisplayTimeout}`);

    if (config.radarSensitivity === undefined) console.warn('[Device] ⚠️ syncConfigUI: radarSensitivity is MISSING from config');
    else console.log(`[Device] syncConfigUI: radarSensitivity = ${config.radarSensitivity}`);
    ...
    // Max Distance - REAL value from device (Gates)
    const maxDist = config.maxDistance;
    document.getElementById('maxDistanceValue').textContent =
        (maxDist !== undefined) ? `${maxDist} (${(maxDist * 0.75).toFixed(1)}m)` : '--';
...
function setupSettingHandlers() {
    ...
    setupSettingPicker('radarSensitivity', 'radarSensitivity',
        // Finer increments: 0-100 in steps of 5 is okay, but user wanted smaller increments.
        // Let's do steps of 1 for precise control.
        Array.from({ length: 101 }, (_, i) => i),
        (v) => `${v}%`,
        'radarSensitivityValue');

    setupSettingPicker('maxDistance', 'maxDistance',
        [2, 3, 4, 5, 6, 7, 8],
        (v) => `${v} (${(v * 0.75).toFixed(1)}m)`,
        'maxDistanceValue');
            ...
            scanBtn.addEventListener('click', () => {
                ...
                networkList.innerHTML = `
                    <div style="text-align: center; padding: var(--spacing-lg); color: var(--text-secondary);">
                        ...
                        <div style="font-size: var(--font-size-xs); color: var(--text-tertiary); margin-top: 4px;">This may take up to 10-15 seconds</div>
                    </div>
...
/**
 * Open Max Distance Picker
 */
function openMaxDistancePopup() {
    const currentGates = DeviceState.state?.maxDistance || 6;
    let selectedGates = currentGates;

    const content = `
        <div class="sleep-log-popup-content">
             <div class="sleep-log-header">
                <span class="sleep-log-header-icon">📏</span>
                <div class="sleep-log-header-text">
                    <div class="sleep-log-header-title">Max Detection Distance</div>
                    <div class="sleep-log-header-subtitle">Limit the radar's analyze range</div>
                </div>
            </div>
            
            <div class="sleep-log-section">
                <div class="sleep-log-time-row" style="justify-content: center;">
                    <div class="sleep-log-time-col">
                        <div class="sleep-log-time-pickers">
                            <div class="picker">
                                <div class="picker-highlight"></div>
                                <div class="picker-scroll" data-picker="max-distance">
                                    <div class="picker-spacer"></div>
                                    ${Array.from({ length: 9 }, (_, i) => `
                                        <div class="picker-item" data-value="${i}">
                                            ${i === 0 ? 'Off' : (i * 0.75).toFixed(2) + 'm (' + i + ' Gates)'}
                                        </div>
                                    `).join('')}
                                    <div class="picker-spacer"></div>
                                </div>
    ...
    const { modal, close } = Modal.create({
        ...
        actions: [
            { label: 'Cancel', primary: false },
            {
                ...
                primary: true,
                onClick: () => {
                    MQTTClient.publishConfig(DeviceState.deviceId, { max_distance: selectedGates });

                    // Optimistic update
                    if (!DeviceState.state) DeviceState.state = {};
                    DeviceState.state.maxDistance = selectedGates;
                    syncConfigUI(DeviceState.state);
    ...
    // Reuse picker logic
    const itemHeight = 44;
    const scroll = modal.querySelector('[data-picker="max-distance"]');

    if (scroll) {
        // Init scroll
        const initialIndex = selectedGates;
        setTimeout(() => scroll.scrollTop = initialIndex * itemHeight, 100);

        // Scroll handler
        let scrollTimeout;
        const handleScroll = () => {
            const index = Math.round(scroll.scrollTop / itemHeight);
            if (index >= 0 && index <= 8) {
                selectedGates = index;
                scroll.querySelectorAll('.picker-item').forEach((item, i) => {
...
function syncConfigUI(config) {
    ...
    // Helper to update checkbox if element exists
    ...
    // Switches
    updateCheck('alarmEnabled', config.alarmEnabled);
    updateCheck('dayIdleEnabled', config.dayIdleEnabled);
    updateCheck('motionEnabled', config.motionEnabled);
    updateCheck('twtEnabled', config.twtEnabled);

    // Timeout Values
    if (config.motionTimeout !== undefined) updateText('motionTimeoutValue', Utils.formatSecondsAsDuration(config.motionTimeout));
    if (config.manualTimeout !== undefined) updateText('manualTimeoutValue', Utils.formatSecondsAsDuration(config.manualTimeout));
    if (config.displayTimeoutSetting !== undefined) updateText('displayTimeoutValue', config.displayTimeoutSetting === 0 ? 'Off' : `${config.displayTimeoutSetting}s`);

    // Sensitivity
    if (config.radarSensitivitySetting !== undefined) updateText('radarSensitivityValue', `${config.radarSensitivitySetting}%`);

    // Max Distance
    if (config.maxDistance !== undefined) {
        // Fix: Explicit undefined check, as 0 is a valid value
        const gates = (config.maxDistance !== null && config.maxDistance !== undefined) ? config.maxDistance : 6;
        const meters = (gates * 0.75).toFixed(2);
        updateText('maxDistanceValue', `${meters}m (${gates} Gates)`);
    }

    // Servo Angles
    if (config.angleOn !== undefined) updateText('angleOnValue', `${config.angleOn}°`);
    if (config.angleOff !== undefined) updateText('angleOffValue', `${config.angleOff}°`);

    // Alarm Time
    if (config.alarmHour !== undefined && config.alarmMin !== undefined) {
        updateText('alarmTime', `${String(config.alarmHour).padStart(2, '0')}:${String(config.alarmMin).padStart(2, '0')}`);
    }

    // Offsets
    if (config.sunriseOffset !== undefined) updateText('sunriseOffsetValue', `${config.sunriseOffset >= 0 ? '+' : ''}${config.sunriseOffset} min`);
    if (config.sunsetOffset !== undefined) updateText('sunsetOffsetValue', `${config.sunsetOffset >= 0 ? '+' : ''}${config.sunsetOffset} min`);

    // City
    if (config.city) updateText('cityValue', config.city);

    // Sleep Target
    if (config.sleepTargetDuration !== undefined) updateText('sleepTargetDurationValue', Utils.formatDuration(config.sleepTargetDuration));
    if (config.sleepTargetBedtimeHour !== undefined && config.sleepTargetBedtimeMin !== undefined) {
        updateText('sleepTargetBedtimeValue', `${String(config.sleepTargetBedtimeHour).padStart(2, '0')}:${String(config.sleepTargetBedtimeMin).padStart(2, '0')}`);
    }
}
...
// Max Distance Handler
document.addEventListener('DOMContentLoaded', () => {
    const maxDistanceSetting = document.getElementById('maxDistanceSetting');
    if (maxDistanceSetting) {
        maxDistanceSetting.addEventListener('click', () => {
            openMaxDistancePopup();
        });
    }
});

...
function syncConfigUI(config) {
    ...
    document.getElementById('servoSpeedValue').textContent =
        (servoSpeed !== undefined && servoSpeed !== null)
            ? (servoSpeed === 0 ? 'Instant' : `${servoSpeed}%`)
            : '--';
            ...
            Modal.confirm('Remove Device', 'Are you sure you want to remove this device from your account? This action cannot be undone and will delete all configuration.', async () => {
                if (Auth && Auth.currentUser && DeviceState && DeviceState.deviceId) {
                    try {
                        ...
                        removeDeviceBtn.innerHTML = '🗑️ Removing...';
                        ...
                        removeDeviceBtn.disabled = false;
                        removeDeviceBtn.innerHTML = '🗑️ Remove Device';
                    }
                } else {
...
function setupServoSpeedUI() {
    ...
    element.addEventListener('click', () => {
        const config = DeviceState.state?.config || {};
        const currentValue = config.servoSpeed ?? 50;
        ...
        const content = `
            ...
            <div style="margin-bottom: var(--spacing-lg);">
                <div style="display: flex; align-items: center; gap: var(--spacing-sm); margin-bottom: var(--spacing-sm);">
                    ...
                    <div>
                        ...
                        <div style="font-size: var(--font-size-sm); color: var(--text-secondary);">
                            Adjust how fast the servo moves between positions.
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="speed-readout">
                <div class="speed-value" id="speedValueDisplay">${selectedSpeed === 0 ? 'Instant' : selectedSpeed + '%'}</div>
                <div class="speed-description" id="speedDescDisplay">
                    ${selectedSpeed === 0 ? 'Moves immediately to the target (loudest)' : 
                      selectedSpeed < 40 ? 'Slower, smoother, and quieter motion' : 
                      selectedSpeed > 70 ? 'Fast and responsive motion' : 
                      'Balanced speed and noise'}
                </div>
            </div>
            ...
            <button class="btn btn-secondary" id="defaultSpeedBtn" style="width: 100%; justify-content: center; gap: var(--spacing-xs);">
                <span style="font-size: 1.2em;">🔄</span> Reset to Default (50%)
            </button>
        `;
            ...
            actions: [
                { label: 'Cancel', primary: false },
                {
                    ...
                    onClick: () => {
                        ...
                        if (displayEl) displayEl.textContent = selectedSpeed === 0 ? 'Instant' : `${selectedSpeed}%`;
                        Toast.success(`Servo speed set to ${selectedSpeed === 0 ? 'Instant' : selectedSpeed + '%'}`);
                    }
                }
            ]
        });
        ...
        const display = modal.querySelector('#speedValueDisplay');
        const descDisplay = modal.querySelector('#speedDescDisplay');
        const defaultBtn = modal.querySelector('#defaultSpeedBtn');

        const updateVisual = (val) => {
            selectedSpeed = parseInt(val);
            display.textContent = selectedSpeed === 0 ? 'Instant' : `${selectedSpeed}%`;
            
            if (selectedSpeed === 0) {
                descDisplay.textContent = 'Moves immediately to the target (loudest)';
            } else if (selectedSpeed < 40) {
                descDisplay.textContent = 'Slower, smoother, and quieter motion';
            } else if (selectedSpeed > 70) {
                descDisplay.textContent = 'Fast and responsive motion';
            } else {
                descDisplay.textContent = 'Balanced speed and noise';
            }
        };
...
function setupSettingPicker(settingId, configKey, values, formatFn, displayId, onSave = null) {
    ...
    const settingMeta = {
        ...
        'radarSensitivity': {
            ...
            tip: '💡 If lights turn off unexpectedly, increase sensitivity. If lights turn on randomly, decrease it.'
        },
        'angleOff': {
            icon: '🔴',
        ...
        'servoSpeed': {
            ...
            tip: '💡 A slower speed produces a quieter, smoother switch action. Set to 0 for the original instant behavior.'
        },
        'motionTimeout': {
            icon: '⏱️',

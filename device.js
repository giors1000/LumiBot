...
function syncConfigUI(config) {
    ...
    // Sunrise/Sunset offsets are now global (hidden elements for backward compat)
    const _globalSunrise = parseInt(localStorage.getItem('zaylo-SunriseOffset') || '0', 10);
    const _globalSunset = parseInt(localStorage.getItem('zaylo-SunsetOffset') || '0', 10);
    const srEl = document.getElementById('sunriseOffsetValue');
    const ssEl = document.getElementById('sunsetOffsetValue');
    if (srEl) srEl.textContent = `${_globalSunrise >= 0 ? '+' : ''}${_globalSunrise} min`;
    if (ssEl) ssEl.textContent = `${_globalSunset >= 0 ? '+' : ''}${_globalSunset} min`;

            ...
            modeChangeTimeout = setTimeout(() => {
                if (pendingModeChange && pendingModeChange.mode === mode) {
                    ...
                    if (mode === 3 && !isSleeping) {
                        ...
                        success = MQTTClient.publishControl(DeviceState.deviceId, { mode: 3, sleep: true, sleepStart: now });
                        if (success) {
                        ...
                        success = MQTTClient.publishControl(DeviceState.deviceId, { mode: mode, sleep: false, sleepEnd: now });
                        if (success) {
                            Toast.success('Sleep session ended');
                        }
                    } else {
                        // STANDARD MODE CHANGE
                        success = MQTTClient.publishControl(DeviceState.deviceId, { mode });
                        if (success) {
    ...
    if (sleepActionBtn) {
        sleepActionBtn.addEventListener('click', () => {
            ...
            if (!isSleeping) {
                ...
                // Send command
                MQTTClient.publishControl(DeviceState.deviceId, { sleep: true, sleepStart: now, mode: 3 });
                ...
                // Send command
                MQTTClient.publishControl(DeviceState.deviceId, { sleep: false, sleepEnd: now, mode: 0 });
...
window.addEventListener('pagehide', () => {
    // Clear the countdown timer interval (runs every 250ms)
    if (DeviceState.timerInterval) {
        clearInterval(DeviceState.timerInterval);
        DeviceState.timerInterval = null;
    }
    // Clear Firestore sync debounce timer

    ...
    try {
        ...
        StateStore.subscribe(DeviceState.deviceId, (state) => {
            ...
            if (state.sleepHistory || state.isSleeping !== undefined) {
                ...
                if (needsFirebaseSync && Auth.getUser()) {
                    DeviceService.updateDevice(Auth.getUser().uid, DeviceState.deviceId, updates)
...
function syncConfigUI(config) {
    ...
    document.getElementById('radarSensitivityValue').textContent =
        (radarSens !== undefined && radarSens !== null) ? `${radarSens}%` : '--';
...
function setupSettingHandlers() {
    ...
    if (radarSensRow) {
        radarSensRow.addEventListener('click', () => {
            const config = DeviceState.state?.config || {};
            let currentVal = config.radarSensitivity ?? 50;
            // Migrate any legacy auto (101) to 50%
                ...
                <div style="margin-bottom: var(--spacing-md, 12px);">
                    <div style="background: rgba(255,255,255,0.04); padding: 14px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08);">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-weight: 500;">
                            ...
                            <span id="sensManualValue" style="color: var(--accent, #6366f1); font-weight: 700; font-size: 1.1em;">${currentVal}%</span>
                        </div>
                        <input type="range" id="sensManualSlider" class="sens-slider" min="0" max="100" value="${currentVal}" step="5">
                        <div style="display: flex; justify-content: space-between; font-size: 0.75em; opacity: 0.5; margin-top: 8px;">
                            <span>Strict (0%)</span>
                            <span>Sensitive (100%)</span>
                        </div>
                        <div style="margin-top: 12px; font-size: 0.8em; color: var(--text-secondary, #aaa); line-height: 1.5;">
                            Lower values require stronger signal for detection (fewer false triggers). Higher values detect weaker signals (more sensitive, may detect through walls).
                        </div>
                    </div>
                </div>
            `;

            let manualVal = currentVal;
                ...
                actions: [
                    { label: 'Cancel', primary: false },
                    {
                        ...
                        onClick: () => {
                            ...
                            if (displayEl) displayEl.textContent = `${manualVal}%`;
                            MQTTClient.publishConfig(DeviceState.deviceId, { radarSensitivity: manualVal });
                            Toast.success(`Sensitivity: ${manualVal}%`);
                        }
                    }
                ]
            });
    ...
    document.querySelector('[data-setting="city"]')?.addEventListener('click', () => {
        ...
        const content = `
            ...
            <div class="input-group">
                ...
                <input type="text" class="modal-input" placeholder="e.g. Bristol,GB or New York,US" 
                ...
                <div style="margin-top: var(--spacing-sm); color: var(--text-tertiary); font-size: var(--font-size-xs);">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-lightbulb"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.9 1.2 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg> Use format: City,CountryCode (GB, US, DE, FR, etc.)
                </div>
            </div>
        `;
        ...
        // Focus input
        setTimeout(() => {
            const el = modal.querySelector('[data-modal-input]');
            if (el) el.focus();
            el.select();
        }, 300);
    });
    ...
    if (removeDeviceBtn) {
        removeDeviceBtn.addEventListener('click', () => {
            Haptic.high();
            Modal.confirm('Remove Device', 'Are you sure you want to remove this device from your account? This action cannot be undone and will delete all configuration.', async () => {
                if (Auth && Auth.currentUser && DeviceState && DeviceState.deviceId) {
                    try {
                        ...
                        const success = await DeviceService.removeDevice(Auth.currentUser.uid, DeviceState.deviceId);
...
function setupSettingPicker(settingId, configKey, values, formatFn, displayId, onSave = null) {
    ...
    const settingMeta = {
        ...
        'radarSensitivity': {
            icon: '📡',
            title: 'Motion Sensitivity',
            description: '0-100% sensitivity. Lower = stricter detection, higher = more sensitive. 50% is a balanced default.',
            tip: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-lightbulb"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.9 1.2 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg> If lights turn off unexpectedly, increase sensitivity. If lights turn on randomly, decrease it.'
        },
        'angleOff': {
        ...
        'sunriseOffset': {
            ...
            tip: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plus"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Positive = after sunrise, ➖ Negative = before sunrise'
        },
        'sunsetOffset': {
            icon: '🌆',
            title: 'Sunset Offset',
            description: 'Adjust when Day Idle mode ends relative to sunset.',
            tip: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plus"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Positive = after sunset, ➖ Negative = before sunset'
        },
        'lightWattage': {
    ...
    if (sleepActionBtn) {
        sleepActionBtn.addEventListener('click', () => {
            ...
            if (!isSleeping) {
                ...
                // Send command
                MQTTClient.publishControl(DeviceState.deviceId, { sleep: false, sleepEnd: now });
...
function openSleepLogPopup(index = null) {
    ...
    const { backdrop, modal, close } = Modal.create({
        ...
        actions: [
            { label: 'Cancel', primary: false },
            {
                label: isEditing ? '💾 Save Changes' : '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plus"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Add Session',
                primary: true,
                onClick: () => {
                    // Validate inputs
                    if (!selectedDate) {
...
function deleteSleepLog(index) {
    ...
    Modal.confirm(
        '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-trash-2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg> Delete Sleep Session',
        `Are you sure you want to delete this sleep session?\n\n` +
        `📅 ${date.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}\n` +
        `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-timer"><line x1="10" x2="14" y1="2" y2="2"/><line x1="12" x2="15" y1="14" y2="11"/><circle cx="12" cy="14" r="8"/></svg> Duration: ${hours}h ${mins}m\n\n` +

...
window.addEventListener('error', (event) => {
  if (typeof originalConsole !== 'undefined') {
    originalConsole.error('[LumiBot] Uncaught error:', event.message, event.filename, event.lineno);
  }
});

window.addEventListener('unhandledrejection', (event) => {
  if (typeof originalConsole !== 'undefined') {
    originalConsole.error('[LumiBot] Unhandled promise rejection:', event.reason);
  }
...
const Theme = {
  ...
  set(theme, save = true) {
    ...
    const color = theme === 'dark' ? '#0a0a0f' : '#f8fafc';
...
const Network = {
  ...
  _handleChange(online) {
    ...
    if (banner) {
      ...
      banner.className = `network-banner visible ${online ? 'online' : 'offline'}`;
      banner.textContent = online ? 'Back online' : 'No internet connection';
  ...
  create(options = {}) {
    ...
    const close = () => {
      ...
      if (document.activeElement && document.activeElement.tagName === 'INPUT') {
        ...
        // Only reset scroll when an input was focused (keyboard was open)
        window.scrollTo(0, 0);
  ...
  input(options = {}) {
    ...
    const content = `
      <div class="input-group">
        <input type="text" class="modal-input" placeholder="${placeholder}" value="${value}" data-modal-input>
      </div>
    `;
...
const Utils = {
  ...
  formatTime(seconds) {
    ...
    const s = Math.ceil(seconds % 60); // Use ceil to avoid 0:00 when there's still <1s
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    ...
    // Ensure body is visible (handles case where CSS might hide it initialy)
    document.body.style.opacity = '1';

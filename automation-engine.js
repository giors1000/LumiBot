...
const AutomationEngine = (function () {
    ...
    function _evaluateRules() {
        ...
        try {
            ...
            savedDevices.forEach(function (device) {
                ...
                // StepperMote and Blind Devices (Servo)
                if (device.type === 'stepper' || device.type === 'blind') {
                    ...
                    // Execute cross-device automations (Radar / Temperature)
                    // These require the web client to bridge SwitchMote data to the Blind/Stepper
                    if (linked) {
                        processPresence(id, rules, config, linked);
                        processHeatProtection(id, rules, config, linked);
                    }

                    // Note: As of StepperMote V6, the firmware autonomously handles time-based automations.
                    // Client-side rule execution (Sunset, Morning, NightLock) is disabled to prevent collisions.
                    return;
                }
            });
        } catch (e) {
    ...
    return {
        ...
        hasActiveRules: function () {
            try {
                var raw = localStorage.getItem('LumiBot-devices');
                if (!raw) return false;

        ...
        try {
            ...
            savedDevices.forEach(function (device) {
                ...
                if (device.type === 'stepper' || device.type === 'blind') {
                    ...
                    // These require the web client to bridge Zaylo data to the Blind/Stepper
                    if (linked) {
                        processPresence(id, rules, config, linked);
                        processHeatProtection(id, rules, config, linked);
                    }

                    // Note: As of Zaylo Slide V6, the firmware autonomously handles time-based automations.

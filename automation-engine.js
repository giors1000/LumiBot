...
const AutomationEngine = (function () {
    ...
    function processMorningWakeUp(deviceId, rules, config) {
        ...
        if (now >= startTime && now <= endTime) {
            ...
            var idealTarget = gradualState.startPos + (targetPos - gradualState.startPos) * progress;

            // Quantize the ideal target into distinct 5% bins to ensure mechanical backlash is overcome.
            // Using Math.round ensures we hit the structural 5% mark as soon as we are halfway there,
            // rather than waiting for the time to fully eclipse it.
            var binSize = 5;
    ...
    function _evaluateRules() {
        ...
        try {
            ...
            savedDevices.forEach(function (device) {
                ...
                // ── Blind (Servo) Devices only ──
                // StepperMote runs automations natively on ESP32
                ...
                if (device.type === 'blind') {
                    ...
                    // Note: As of StepperMote V6, the firmware autonomously handles all automations.
                    // Client-side rule execution is disabled to prevent manual-override collisions
                    // with the C++ engine (which uses its own NTP logic).
                    return;
                }
            });
        } catch (e) {
